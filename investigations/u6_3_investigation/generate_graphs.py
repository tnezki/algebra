...
import matplotlib.pyplot as plt
fs=[
[{'expr':lambda x:x**2-4,'deriv':lambda x:2*x,'color':'steelblue'}],
[{'expr':lambda x:(x-1)**2,'deriv':lambda x:2*(x-1),'color':'steelblue'}],
[{'expr':lambda x:x**2+3,'deriv':lambda x:2*x,'color':'steelblue'}],
[{'expr':lambda x:-x**2+4,'deriv':lambda x:-2*x,'color':'steelblue'}]]
fig=make_2x2_grid(fs,titles=['A','B','C','D'])
path=os.path.join(OUTPUT_DIR,'root_types.png'); fig.savefig(path,dpi=144,bbox_inches='tight'); print('Saved:',path); plt.close(fig)
