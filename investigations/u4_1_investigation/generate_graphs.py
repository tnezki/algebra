...
import matplotlib.pyplot as plt
fs=[
[{'expr':lambda x:x,'deriv':lambda x:0*x+1,'color':'steelblue'}],
[{'expr':lambda x:x**2,'deriv':lambda x:2*x,'color':'steelblue'}],
[{'expr':lambda x:2**x,'deriv':lambda x:np.log(2)*2**x,'color':'steelblue'}],
[{'expr':lambda x:np.abs(x),'deriv':lambda x:np.where(x<0,-1,1),'color':'steelblue'}]]
fig=make_2x2_grid(fs,titles=['A','B','C','D'])
path=os.path.join(OUTPUT_DIR,'families.png'); fig.savefig(path,dpi=144,bbox_inches='tight'); print('Saved:',path); plt.close(fig)
