...
# ---- ALGEBRA INVESTIGATION FIGURES APPENDED BELOW ----

import matplotlib.pyplot as plt
f1=[{'expr':lambda x:x+1,'deriv':lambda x:0*x+1,'color':'steelblue','label':'A'},{'expr':lambda x:-x+5,'deriv':lambda x:0*x-1,'color':'firebrick','label':'B'}]
f2=[{'expr':lambda x:0.5*x+1,'deriv':lambda x:0*x+0.5,'color':'steelblue','label':'A'},{'expr':lambda x:0.5*x+4,'deriv':lambda x:0*x+0.5,'color':'firebrick','label':'B'}]
fig=make_2x1_grid([f1,f2],titles=['System A','System B'])
path=os.path.join(OUTPUT_DIR,'system_types.png'); fig.savefig(path,dpi=144,bbox_inches='tight'); print('Saved:',path); plt.close(fig)
