...
import matplotlib.pyplot as plt
f1=[{'expr':lambda x:2**x,'deriv':lambda x:np.log(2)*2**x,'color':'steelblue'}]
f2=[{'expr':lambda x:0.5**x,'deriv':lambda x:np.log(0.5)*0.5**x,'color':'steelblue'}]
fig=make_2x1_grid([f1,f2],titles=['Growth','Decay'])
path=os.path.join(OUTPUT_DIR,'exponential_pair.png'); fig.savefig(path,dpi=144,bbox_inches='tight'); print('Saved:',path); plt.close(fig)
