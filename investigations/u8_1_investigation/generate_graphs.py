...
[{'expr':lambda x:x**2+1,'deriv':lambda x:2*x,'color':'steelblue'}],
[{'expr':lambda x:2**x,'deriv':lambda x:np.log(2)*2**x,'color':'steelblue'}]]
fig=make_3x1_grid(fs,titles=['Linear','Quadratic','Exponential'])
path=os.path.join(OUTPUT_DIR,'model_families.png'); fig.savefig(path,dpi=144,bbox_inches='tight'); print('Saved:',path); plt.close(fig)
