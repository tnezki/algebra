...
import matplotlib.pyplot as plt
fs=[
[{'expr':lambda x:2*x**2,'deriv':lambda x:4*x,'color':'steelblue'}],
[{'expr':lambda x:0.5*x**2,'deriv':lambda x:x,'color':'steelblue'}],
[{'expr':lambda x:-x**2,'deriv':lambda x:-2*x,'color':'steelblue'}],
[{'expr':lambda x:-2*np.abs(x),'deriv':lambda x:np.where(x<0,2,-2),'color':'steelblue'}]]
fig=make_2x2_grid(fs,titles=['A','B','C','D'])
path=os.path.join(OUTPUT_DIR,'transforms.png'); fig.savefig(path,dpi=144,bbox_inches='tight'); print('Saved:',path); plt.close(fig)
