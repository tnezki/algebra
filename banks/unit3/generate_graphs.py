...
# UNIT 3 GENERATION CALLS - appended to the authoritative Framework tool.
if __name__ == '__main__':
    def line_file(filename,m,b,context=None,label=None):
        fig,ax=plt.subplots(figsize=(3.5,3.5))
        funcs=[{'expr':lambda x,m=m,b=b:m*x+b,'deriv':lambda x,m=m:np.zeros_like(x)+m,'color':'steelblue','label':label}]
        if context:
            make_context_graph(ax,funcs,*context)
        else:
            make_standard_graph(ax,funcs)
        save_graph(fig,filename); plt.close(fig)
    def compare_file(filename,m1,b1,m2,b2,context=None,labels=('A','B')):
        fig,ax=plt.subplots(figsize=(3.5,3.5))
        funcs=[{'expr':lambda x,m=m1,b=b1:m*x+b,'deriv':lambda x,m=m1:np.zeros_like(x)+m,'color':'steelblue','label':labels[0]},
               {'expr':lambda x,m=m2,b=b2:m*x+b,'deriv':lambda x,m=m2:np.zeros_like(x)+m,'color':'firebrick','label':labels[1]}]
        if context: make_context_graph(ax,funcs,*context)
        else: make_standard_graph(ax,funcs)
        save_graph(fig,filename); plt.close(fig)
    def ineq_file(filename,ineqs,title=''):
        fig,ax=plt.subplots(figsize=(3.5,3.5)); make_inequality_graph(ax,ineqs,title=title); save_graph(fig,filename); plt.close(fig)
    def types_file(filename):
        fig,axs=plt.subplots(1,3,figsize=(9,3))
        specs=[[(1,1),(-1,5)],[(1,1),(1,4)],[(1,1),(1,1)]]
        titles=['A','B','C']
        for ax,spec,title in zip(axs,specs,titles):
            funcs=[]
            for j,(m,b) in enumerate(spec):
                funcs.append({'expr':lambda x,m=m,b=b:m*x+b,'deriv':lambda x,m=m:np.zeros_like(x)+m,'color':['steelblue','firebrick'][j]})
            make_standard_graph(ax,funcs,title=title)
        fig.tight_layout(); save_graph(fig,filename); plt.close(fig)
    # 3.1 systems
    compare_file('u3_s31_wtc_break_even.png',6,20,3,44,(0,12,0,90,'Hours','Cost ($)','Room-rental break-even'),('Club A','Club B'))
    compare_file('u3_s31_note_intersection.png',1,1,-1,5)
    types_file('u3_s31_note_types.png'); types_file('u3_s31_types_practice.png')
    for f,(x,y,m1,m2) in enumerate([(1,2,1,-1),(2,-1,2,-1),(-2,3,1,3),(3,1,-1,2)],1):
        compare_file(f'u3_s31_practice_f{f}.png',m1,y-m1*x,m2,y-m2*x)
    # Review lines
    line_file('u3_review_s31_line.png',1,1); line_file('u3_review_s32_line.png',-2,4)
    # 3.3 context system graphs
    compare_file('u3_s33_wtc_tickets.png',-1,120,-1.6,156,(0,120,0,130,'Adult tickets','Student tickets','Ticket constraints'),('Count total','Revenue'))
    compare_file('u3_s33_note_break_even.png',4,10,2,22,(0,10,0,55,'Uses','Cost ($)','Plan break-even'),('Plan A','Plan B'))
    compare_file('u3_s33_practice_f1.png',5,10,2,22,(0,10,0,55,'Input','Output','Modeled system'))
    compare_file('u3_s33_practice_f2.png',4,8,2,20,(0,10,0,55,'Input','Output','Modeled system'))
    compare_file('u3_s33_practice_f3.png',3,15,1,25,(0,10,0,55,'Input','Output','Modeled system'))
    compare_file('u3_s33_practice_f4.png',6,5,3,20,(0,10,0,70,'Input','Output','Modeled system'))
    # 3.4 single inequalities
    ineq_file('u3_s34_note_ineq.png',[{'expr':lambda x:x-2,'deriv':lambda x:np.ones_like(x),'color':'steelblue','shade':'above','inclusive':True}],r'$y\geq x-2$')
    ineq_file('u3_s34_note_reverse.png',[{'expr':lambda x:-x+3,'deriv':lambda x:-np.ones_like(x),'color':'steelblue','shade':'below','inclusive':False}],r'$y<-x+3$')
    ineq_file('u3_s34_reverse_practice.png',[{'expr':lambda x:x+1,'deriv':lambda x:np.ones_like(x),'color':'steelblue','shade':'above','inclusive':False}],r'$y>x+1$')
    ineq_file('u3_s34_wtc_budget.png',[{'expr':lambda x:-1.5*x+15,'deriv':lambda x:-1.5*np.ones_like(x),'color':'steelblue','shade':'below','inclusive':True}],r'$6x+4y\leq60$')
    specs=[(1,2,'below',True),(-1,4,'above',False),(2,-3,'below',False),(-2,5,'above',True)]
    for f,(m,b,shade,incl) in enumerate(specs,1):
        ineq_file(f'u3_s34_practice_f{f}.png',[{'expr':lambda x,m=m,b=b:m*x+b,'deriv':lambda x,m=m:np.zeros_like(x)+m,'color':'steelblue','shade':shade,'inclusive':incl}])
    # Additional reverse-translation inequality figures
    rev_specs=[(-1,3,'above',False),(2,-1,'below',True),(1,2,'above',True),(-2,4,'below',False)]
    for f,(m,b,shade,incl) in enumerate(rev_specs,1):
        ineq_file(f'u3_s34_reverse_f{f}.png',[{'expr':lambda x,m=m,b=b:m*x+b,'deriv':lambda x,m=m:np.zeros_like(x)+m,'color':'steelblue','shade':shade,'inclusive':incl}])
    # 3.5 systems of inequalities
    ineq_file('u3_s35_note_overlap.png',[{'expr':lambda x:-x+8,'deriv':lambda x:-np.ones_like(x),'color':'steelblue','shade':'below','inclusive':True},{'expr':lambda x:0*x+2,'deriv':lambda x:np.zeros_like(x),'color':'firebrick','shade':'above','inclusive':True}])
    ineq_file('u3_s35_wtc_feasible.png',[{'expr':lambda x:-x+10,'deriv':lambda x:-np.ones_like(x),'color':'steelblue','shade':'below','inclusive':True},{'expr':lambda x:0*x+4,'deriv':lambda x:np.zeros_like(x),'color':'firebrick','shade':'above','inclusive':True}])
    sys_specs=[((-1,8,'below',True),(0,2,'above',True)),((2,1,'below',True),(-1,4,'above',True)),((1,-2,'above',True),(-1,6,'below',True)),((-2,9,'below',True),(0,3,'above',True))]
    for f,pair in enumerate(sys_specs,1):
        ineqs=[]
        for j,(m,b,shade,incl) in enumerate(pair):
            ineqs.append({'expr':lambda x,m=m,b=b:m*x+b,'deriv':lambda x,m=m:np.zeros_like(x)+m,'color':['steelblue','firebrick'][j],'shade':shade,'inclusive':incl})
        ineq_file(f'u3_s35_practice_f{f}.png',ineqs)
    ineq_file('u3_s35_empty_practice.png',[{'expr':lambda x:0*x+5,'deriv':lambda x:np.zeros_like(x),'color':'steelblue','shade':'above','inclusive':True},{'expr':lambda x:0*x+2,'deriv':lambda x:np.zeros_like(x),'color':'firebrick','shade':'below','inclusive':True}])
    ineq_file('u3_s35_note_empty.png',[{'expr':lambda x:0*x+4,'deriv':lambda x:np.zeros_like(x),'color':'steelblue','shade':'above','inclusive':True},{'expr':lambda x:0*x+1,'deriv':lambda x:np.zeros_like(x),'color':'firebrick','shade':'below','inclusive':True}])
    # Secure representation graphs V1-V4
    for v,m,b in [(1,1,2),(2,-2,4),(3,2,-3),(4,3,1)]: line_file(f'u3_sum_v{v}_line.png',m,b)
    # Recurring Diamond routine
    make_diamond(r'$-18$','','',r'$-3$',filename='diamond_31.png')
    make_diamond(r'$20$','','',r'$9$',filename='diamond_32.png')
    make_diamond(r'$-24$','','',r'$-2$',filename='diamond_33.png')
    make_diamond(r'$28$','','',r'$11$',filename='diamond_34.png')
    make_diamond(r'$-21$','','',r'$4$',filename='diamond_35.png')
