...
# CANONICAL UNIT FIGURE GENERATION — appended below the approved framework tool.
if __name__ == '__main__':
    import matplotlib.pyplot as plt
    def fn(expr,deriv,color='steelblue',label=None):
        return {'expr':expr,'deriv':deriv,'color':color,'label':label}
    def linear(m,b,color='steelblue',label=None):
        return fn(lambda x,m=m,b=b:m*x+b, lambda x,m=m:np.zeros_like(x)+m, color, label)
    def quad(a=1,h=0,k=0,color='steelblue',label=None):
        return fn(lambda x,a=a,h=h,k=k:a*(x-h)**2+k, lambda x,a=a,h=h:2*a*(x-h), color, label)
    def expo(a=2,b=1.5,k=0,color='steelblue',label=None):
        return fn(lambda x,a=a,b=b,k=k:a*(b**x)+k, lambda x,a=a,b=b:a*(b**x)*np.log(b), color, label)
    def absv(a=1,h=0,k=0,color='steelblue',label=None):
        return fn(lambda x,a=a,h=h,k=k:a*np.abs(x-h)+k, lambda x,a=a,h=h:a*np.where(x<h,-1,1), color, label)

    fig0,ax=plt.subplots(figsize=(5.4,5.4)); make_standard_graph(ax,[linear(2,-1,'steelblue','A'),linear(-2,3,'firebrick','B')],''); save_graph(fig0,'u3_s1_wtc.png'); plt.close(fig0)
    fig0,ax=plt.subplots(figsize=(5.4,5.4)); make_standard_graph(ax,[linear(3,-2,'steelblue','A'),linear(-3,4,'firebrick','B')],''); save_graph(fig0,'u3_s2_wtc.png'); plt.close(fig0)
    fig0,ax=plt.subplots(figsize=(5.4,5.4)); make_standard_graph(ax,[quad(-0.25,1,1)],''); save_graph(fig0,'u3_s3_wtc.png'); plt.close(fig0)
    fig0,ax=plt.subplots(figsize=(5.4,5.4)); make_inequality_graph(ax,[{'expr':lambda x:2*x-0,'deriv':lambda x:np.zeros_like(x)+2,'color':'steelblue','shade':'above','inclusive':False}],''); save_graph(fig0,'u3_s4_wtc.png'); plt.close(fig0)
    fig0,ax=plt.subplots(figsize=(5.4,5.4)); make_inequality_graph(ax,[{'expr':lambda x:x+2,'deriv':lambda x:np.zeros_like(x)+1,'color':'steelblue','shade':'above','inclusive':True},{'expr':lambda x:-0.5*x+7,'deriv':lambda x:np.zeros_like(x)-0.5,'color':'firebrick','shade':'below','inclusive':False}],''); save_graph(fig0,'u3_s5_wtc.png'); plt.close(fig0)
