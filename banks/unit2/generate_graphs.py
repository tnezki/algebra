...
# UNIT 2 CANONICAL FIGURE GENERATION — appended below the approved marker.
if __name__ == '__main__':
    import matplotlib.pyplot as plt
    # Type 1 helpers
    def lin(m,b,color='steelblue',label=None):
        return {'expr':lambda x,m=m,b=b: m*x+b,'deriv':lambda x,m=m: np.zeros_like(x)+m,'color':color,'label':label}
    specs=[
      ('u2_s1_wtc_slope.png',[lin(2,1)],''),
      ('u2_s1_note_slope.png',[lin(-1.5,4)],''),
      ('u2_s2_wtc_parallel.png',[lin(1.5,-2,'steelblue','A'),lin(1.5,3,'firebrick','B')],''),
      ('u2_s2_note_line.png',[lin(-2,5)],''),
      ('u2_s3_wtc_twopoints.png',[lin(1.5,-1)],''),
      ('u2_s4_note_graph.png',[lin(2,4)],''),
      ('u2_sum_v5_graph.png',[lin(-1,6)],''),
    ]
    for name,funcs,title in specs:
        fig0,ax=plt.subplots(figsize=(5.4,5.4)); make_standard_graph(ax,funcs,title); save_graph(fig0,name); plt.close(fig0)
    fig0,ax=plt.subplots(figsize=(5.8,4.6)); make_context_graph(ax,[lin(3,8)],0,8,0,36,xlabel='Hours',ylabel='Total cost (dollars)',title=''); save_graph(fig0,'u2_s3_note_context.png'); plt.close(fig0)
    # Type 9 two-panel display with different visual scales to foreground scale checking.
    fig0,axs=plt.subplots(1,2,figsize=(10.5,4.7));
    make_context_graph(axs[0],[lin(2,0)],0,6,0,14,xlabel='Input',ylabel='Output',title='Graph A')
    make_context_graph(axs[1],[lin(2,0)],0,30,0,70,xlabel='Input',ylabel='Output',title='Graph B')
    fig0.tight_layout(); save_graph(fig0,'u2_s4_wtc_scale.png'); plt.close(fig0)
    # Scatter figures
    scatter_specs=[
      ('u2_s5_wtc_scatter.png',[1,2,3,4,5,6,7,8],[5,8,10,13,14,18,19,23],True,'Practice sessions','Score increase'),
      ('u2_s5_note_scatter.png',[0,1,2,3,4,5,6,7],[12,15,17,21,22,26,28,31],True,'Weeks','Plant height (cm)'),
      ('u2_ext07_scatter.png',[1,2,3,4,5,6,7,8,9],[7,9,12,13,17,18,21,24,25],True,'Time','Measured value'),
      ('u2_sum_v6_scatter.png',[1,2,3,4,5,6,7],[10,13,14,18,19,23,25],True,'Input','Observed output'),
    ]
    for name,xs,ys,fit,xlab,ylab in scatter_specs:
        fig0,ax=plt.subplots(figsize=(6.2,4.8)); make_scatter_plot(ax,xs,ys,0,max(xs)+1,0,max(ys)+5,line_of_best_fit=fit,xlabel=xlab,ylabel=ylab,title=''); save_graph(fig0,name); plt.close(fig0)
