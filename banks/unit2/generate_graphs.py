...
# UNIT 2 GENERATION CALLS — appended without modifying the authoritative tool above.
if __name__ == '__main__':
    def line_file(filename,m,b,context=None):
        fig,ax=plt.subplots(figsize=(3.5,3.5))
        funcs=[{'expr':lambda x,m=m,b=b:m*x+b,'deriv':lambda x,m=m:np.zeros_like(x)+m,'color':'steelblue'}]
        if context:
            make_context_graph(ax,funcs,context[0],context[1],context[2],context[3],xlabel=context[4],ylabel=context[5],title=context[6])
        else: make_standard_graph(ax,funcs)
        save_graph(fig,filename); plt.close(fig)
    line_file('u2_s21_wtc_cyclist.png',0.2,1,(0,30,0,8,'Time (min)','Distance (mi)','Cyclist trip'))
    line_file('u2_s21_note_line.png',1.5,1)
    for f,m,b in [(1,2,-1),(2,-1,3),(3,0.5,2),(4,-2,5)]: line_file(f'u2_s21_practice_f{f}.png',m,b)
    line_file('u2_s22_wtc_service.png',2,6,(0,8,0,24,'Months','Cost ($)','Music service cost'))
    line_file('u2_s22_note_line.png',-0.5,2)
    line_file('u2_s22_predict_line.png',3,1)
    for f,m,b in [(1,2,-1),(2,-2,4),(3,0.5,-2),(4,-1,3)]: line_file(f'u2_s22_practice_f{f}.png',m,b)
    line_file('u2_s23_wtc_tank.png',3,14,(0,6,10,34,'Time (min)','Water (gal)','Water in tank'))
    line_file('u2_s23_note_line.png',2,-4)
    for f,m,b in [(1,2,1),(2,-2,4),(3,0.5,-3),(4,-1,3)]: line_file(f'u2_s23_practice_f{f}.png',m,b)
    def compare_file(filename,m1,b1,m2,b2):
        fig,ax=plt.subplots(figsize=(3.5,3.5)); funcs=[
          {'expr':lambda x,m=m1,b=b1:m*x+b,'deriv':lambda x,m=m1:np.zeros_like(x)+m,'color':'steelblue','label':'A'},
          {'expr':lambda x,m=m2,b=b2:m*x+b,'deriv':lambda x,m=m2:np.zeros_like(x)+m,'color':'firebrick','label':'B'}]
        make_standard_graph(ax,funcs); save_graph(fig,filename); plt.close(fig)
    fig,ax=plt.subplots(figsize=(3.5,3.5)); funcs=[
      {'expr':lambda x:6*x+12,'deriv':lambda x:np.zeros_like(x)+6,'color':'steelblue','label':'Shop A'},
      {'expr':lambda x:5*x+20,'deriv':lambda x:np.zeros_like(x)+5,'color':'firebrick','label':'Shop B'}]
    make_context_graph(ax,funcs,0,10,0,80,xlabel='Hours',ylabel='Cost ($)',title='Bike-rental cost comparison'); save_graph(fig,'u2_s24_wtc_compare.png'); plt.close(fig)
    compare_file('u2_s24_note_compare.png',2,-2,1,3)
    for f,spec in [(1,(2,-2,1,3)),(2,(2,4,-1,-3)),(3,(1,1,-1,5)),(4,(3,-5,2,1))]: compare_file(f'u2_s24_practice_f{f}.png',*spec)
    def scatter_file(filename,m,b,seed):
        x=np.array([0,1,2,3,4,5,6,7],dtype=float); noise=np.array([0,1,-1,0.5,-0.5,1,-1,0.5]); y=m*x+b+noise*((seed%3)+0.7)
        fig,ax=plt.subplots(figsize=(3.5,3.5)); make_scatter_plot(ax,x,y,0,8,min(0,b-4),max(y)+4,line_of_best_fit=True,xlabel='x',ylabel='y',title='Linear data trend'); save_graph(fig,filename); plt.close(fig)
    # WTC uses temperature-like range separately
    fig,ax=plt.subplots(figsize=(3.5,3.5)); x=np.array([55,60,65,70,75,80]); y=np.array([18,21,25,28,31,35]); make_scatter_plot(ax,x,y,50,85,15,40,line_of_best_fit=True,xlabel='Temperature (F)',ylabel='Drinks sold',title='Lunch sales trend'); save_graph(fig,'u2_s25_wtc_scatter.png'); plt.close(fig)
    scatter_file('u2_s25_note_scatter.png',2,2,1)
    for f,m,b in [(1,2,5),(2,3,8),(3,-1,20),(4,4,2)]: scatter_file(f'u2_s25_practice_f{f}.png',m,b,f)
    for v,m,b in [(1,2,-1),(2,-1,4),(3,0.5,2),(4,-2,5)]: line_file(f'u2_sum_v{v}_line.png',m,b)
    for v in [5,6]: scatter_file(f'u2_sum_v{v}_scatter.png',2,8,v)
    make_diamond(r'$-12$','','',r'$-1$',filename='diamond_21.png')
    make_diamond(r'$18$','','',r'$9$',filename='diamond_22.png')
    make_diamond(r'$-20$','','',r'$-1$',filename='diamond_23.png')
    make_diamond(r'$24$','','',r'$10$',filename='diamond_24.png')
    make_diamond(r'$-15$','','',r'$2$',filename='diamond_25.png')
