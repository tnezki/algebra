...
if __name__ == '__main__':
    # U1-S1-WTC01 — context relationship
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    make_context_graph(ax, [
        {'expr': lambda x: 2*x+3, 'deriv': lambda x: np.full_like(x,2.0), 'color':'steelblue', 'label':None}
    ], xmin=0, xmax=6, ymin=0, ymax=16, xlabel='Time', ylabel='Distance', title='')
    save_graph(fig, 'u1_s1_wtc_context.png'); plt.close(fig)

    # U1-S1-NOTE-EX02 — standard graph
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    make_standard_graph(ax, [
        {'expr': lambda x: -x+4, 'deriv': lambda x: np.full_like(x,-1.0), 'color':'steelblue', 'label':None}
    ], title='')
    save_graph(fig, 'u1_s1_note_repr.png'); plt.close(fig)

    # U1-S3-WTC01 — four family shapes
    fams=[
        [{'expr':lambda x:x+1,'deriv':lambda x:np.ones_like(x),'color':'steelblue','label':None}],
        [{'expr':lambda x:0.35*x**2-2,'deriv':lambda x:0.7*x,'color':'steelblue','label':None}],
        [{'expr':lambda x:1.4**x-2,'deriv':lambda x:(1.4**x)*np.log(1.4),'color':'steelblue','label':None}],
        [{'expr':lambda x:np.abs(x)-3,'deriv':lambda x:np.where(x>=0,1.0,-1.0),'color':'steelblue','label':None}],
    ]
    fig=make_2x2_grid(fams, titles=['A','B','C','D'])
    save_graph(fig,'u1_s3_wtc_families.png'); plt.close(fig)

    # U1-S3-NOTE-EX03 — compare quadratic and exponential
    fig=make_2x1_grid([
        [{'expr':lambda x:0.4*x**2-2,'deriv':lambda x:0.8*x,'color':'steelblue','label':None}],
        [{'expr':lambda x:1.45**x-2,'deriv':lambda x:(1.45**x)*np.log(1.45),'color':'steelblue','label':None}],
    ], titles=['A','B'])
    save_graph(fig,'u1_s3_note_compare.png'); plt.close(fig)

    # U1-S4-WTC01 — algebra tiles
    make_algebra_tiles({'x':3,'one':2,'neg_one':1}, filename='u1_s4_wtc_tiles.png')
    plt.close('all')

    # U1-S4-NOTE-EX03 — area model
    make_rectangle_model([r'$x$',r'$3$'], [r'$x$',r'$2$'],
                         [[r'$x^2$',r'$2x$'],[r'$3x$',r'$6$']],
                         filename='u1_s4_note_area.png')
    plt.close('all')

    # U1-EXT09 — bounded interval -1 < x <= 5
    fig, ax = plt.subplots(figsize=(3.5,1.25))
    make_number_line(ax,[{'start':-1,'end':5,'start_open':True,'end_open':False,'direction':None,'color':'steelblue'}], xmin=-8, xmax=8)
    save_graph(fig,'u1_ext09_numline.png'); plt.close(fig)

    # Summative V5 slot 08 — x >= -2
    fig, ax = plt.subplots(figsize=(3.5,1.25))
    make_number_line(ax,[{'start':-2,'end':-2,'start_open':False,'end_open':False,'direction':'right','color':'steelblue'}], xmin=-8, xmax=8)
    save_graph(fig,'u1_sum_v5_q08_numline.png'); plt.close(fig)

    # Summative V6 slot 08 — x < 4
    fig, ax = plt.subplots(figsize=(3.5,1.25))
    make_number_line(ax,[{'start':4,'end':4,'start_open':True,'end_open':True,'direction':'left','color':'steelblue'}], xmin=-8, xmax=8)
    save_graph(fig,'u1_sum_v6_q08_numline.png'); plt.close(fig)
