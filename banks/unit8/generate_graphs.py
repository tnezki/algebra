...
# ================= UNIT 8 GENERATION CALLS (3d REPRESENTATION UPGRADE) =================
...
def _save_scatter(filename,x,y,xmin,xmax,ymin,ymax,fit=False,xlabel='x',ylabel='y'):
    fig, ax = plt.subplots(figsize=(5.4,4.3))
    make_scatter_plot(ax,x,y,xmin,xmax,ymin,ymax,line_of_best_fit=fit,xlabel=xlabel,ylabel=ylabel,title='')
    fig.savefig(os.path.join(OUTPUT_DIR, filename), dpi=144, bbox_inches='tight')
    plt.close(fig)

def _save_context(filename, expr, deriv, xmax, ymax):
    fig, ax = plt.subplots(figsize=(5.6,4.5))
    make_context_graph(ax,[{'expr':expr,'deriv':deriv,'color':'steelblue'}],0,xmax,0,ymax,xlabel='Time',ylabel='Height',title='')
    fig.savefig(os.path.join(OUTPUT_DIR, filename), dpi=144, bbox_inches='tight')
    plt.close(fig)

# Retained single-use originals.
_save_grid4('u8_s1_family_gallery.png', [[{'expr': lambda x: x+2, 'deriv': lambda x: np.ones_like(x), 'color':'steelblue'}],[{'expr': lambda x: 0.5*(x-1)**2-2, 'deriv': lambda x: x-1, 'color':'steelblue'}],[{'expr': lambda x: 2**(x/2)-2, 'deriv': lambda x: (np.log(2)/2)*2**(x/2), 'color':'steelblue'}],[{'expr': lambda x: np.abs(x)-3, 'deriv': lambda x: np.where(x<0,-1,1), 'color':'steelblue'}]])
_save_grid2('u8_s2_quad_pair.png', [[{'expr': lambda x: x**2, 'deriv': lambda x: 2*x, 'color':'steelblue'}],[{'expr': lambda x: (x-2)**2-1, 'deriv': lambda x: 2*(x-2), 'color':'steelblue'}]])
_save_grid2('u8_s2_abs_pair.png', [[{'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x<0,-1,1), 'color':'steelblue'}],[{'expr': lambda x: -2*np.abs(x)+1, 'deriv': lambda x: np.where(x<0,2,-2), 'color':'steelblue'}]])
_save_grid2('u8_s2_exp_pair.png', [[{'expr': lambda x: 2**x, 'deriv': lambda x: np.log(2)*2**x, 'color':'steelblue'}],[{'expr': lambda x: 2**(x-1)+2, 'deriv': lambda x: np.log(2)*2**(x-1), 'color':'steelblue'}]])
_save_grid4('u8_s2_multi_transform.png', [[{'expr': lambda x: x**2, 'deriv': lambda x: 2*x, 'color':'steelblue'}],[{'expr': lambda x: (x-2)**2+2, 'deriv': lambda x: 2*(x-2), 'color':'steelblue'}],[{'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x<0,-1,1), 'color':'steelblue'}],[{'expr': lambda x: np.abs(x-2)+2, 'deriv': lambda x: np.where(x<2,-1,1), 'color':'steelblue'}]])
_save_std('u8_s3_abs_graph.png', [{'expr': lambda x: np.abs(x-2)-3, 'deriv': lambda x: np.where(x<2,-1,1), 'color':'steelblue'}])
_save_scatter('u8_s4_scatter_linear.png',[0,1,2,3,4,5],[1,5,9,13,17,21],0,6,0,34,True,'time','distance')
_save_scatter('u8_s4_scatter_quad.png',[-3,-2,-1,0,1,2,3],[9,4,1,0,1,4,9],-4,4,-1,10,False,'x','y')
_save_scatter('u8_s4_scatter_exp.png',[0,1,2,3,4,5],[3,5,8,14,24,41],0,6,0,45,False,'x','y')
_save_grid4('u8_s5_family_gallery.png', [[{'expr': lambda x: 1.5*x-1, 'deriv': lambda x: 1.5*np.ones_like(x), 'color':'steelblue'}],[{'expr': lambda x: -(x-1)**2+4, 'deriv': lambda x: -2*(x-1), 'color':'steelblue'}],[{'expr': lambda x: 1.5**x-2, 'deriv': lambda x: np.log(1.5)*1.5**x, 'color':'steelblue'}],[{'expr': lambda x: 2*np.abs(x+1)-2, 'deriv': lambda x: np.where(x<-1,-2,2), 'color':'steelblue'}]])
make_diamond('$-15$', '$-3$', '$5$', '$2$', filename='u8_s1_diamond.png')
make_diamond('$-24$', '$4$', '$-6$', '$-2$', filename='u8_s2_diamond.png')
make_diamond('$14$', '$-2$', '$-7$', '$-9$', filename='u8_s3_diamond.png')
make_diamond('$24$', '$3$', '$8$', '$11$', filename='u8_s4_diamond.png')
make_diamond('$-24$', '$-4$', '$6$', '$2$', filename='u8_s5_diamond.png')

# 8.1 distinct family-gallery instances. Positions stay TL linear, TR quadratic, BL exponential, BR absolute value.
_s1_gallery_specs = [
('u8_s1_ex_06_family_gallery.png', 1.0,-1, 0.5,-2,1, 1.5,-1, 1.0,1,-2),
('u8_s1_yti_06_family_gallery.png',-1.0,2,-0.5,1,-1, 2.0,-2, 1.5,-2,1),
('u8_s1_p1_05_family_gallery.png',0.5,3, 1.0,0,-3, 1.7,-1, 0.5,2,-1),
('u8_s1_p1_15_family_gallery.png',-0.75,-2, 0.5,2,-1, 1.4,0, 2.0,-1,-3),
('u8_s1_p2_05_family_gallery.png',1.25,0,-1.0,-1,4, 1.6,-3, 1.0,3,-2),
('u8_s1_p2_15_family_gallery.png',-0.5,1, 0.75,1,-4, 1.8,-2, 1.5,0,-1),
('u8_s1_p3_05_family_gallery.png',1.5,-3,-0.5,2,3, 1.3,-1, 0.75,-3,0),
('u8_s1_p3_15_family_gallery.png',0.75,2, 0.5,-2,-2, 2.0,-4, 1.25,2,-3),
('u8_s1_px_05_family_gallery.png',-1.25,1, 1.0,1,-5, 1.5,-2, 0.5,-1,2),
('u8_s1_px_15_family_gallery.png',0.6,-2,-0.75,-2,4, 1.7,-3, 1.75,1,-2),
('u8_s1_cyu_05_family_gallery.png',-0.6,3, 0.5,3,-3, 1.4,-2, 1.0,-2,-4),
]
for fn,m,b,a,h,k,base,eshift,c,ah,ak in _s1_gallery_specs:
    _save_grid4(fn, [[{'expr': lambda x,m=m,b=b:m*x+b,'deriv':lambda x,m=m:m*np.ones_like(x),'color':'steelblue'}],
                     [{'expr': lambda x,a=a,h=h,k=k:a*(x-h)**2+k,'deriv':lambda x,a=a,h=h:2*a*(x-h),'color':'steelblue'}],
                     [{'expr': lambda x,base=base,eshift=eshift:base**(x/2)+eshift,'deriv':lambda x,base=base:(np.log(base)/2)*base**(x/2),'color':'steelblue'}],
                     [{'expr': lambda x,c=c,ah=ah,ak=ak:c*np.abs(x-ah)+ak,'deriv':lambda x,c=c,ah=ah:np.where(x<ah,-c,c),'color':'steelblue'}]])

# 8.2 exponential pair variations.
_exp_specs={
'u8_s2_p1_15_exp_pair.png':(2,1),'u8_s2_p2_15_exp_pair.png':(-1,-2),'u8_s2_p3_15_exp_pair.png':(1,-3),'u8_s2_px_15_exp_pair.png':(-2,1),'u8_s2_cyu_05_exp_pair.png':(3,2),
'u8_s2_p1_20_exp_pair.png':(0,-2),'u8_s2_p2_20_exp_pair.png':(2,3),'u8_s2_p3_20_exp_pair.png':(-1,1),'u8_s2_px_20_exp_pair.png':(3,-1),'u8_s2_wu1_02_exp_pair.png':(-2,2),'u8_s2_etb_03_exp_pair.png':(1,4)}
for fn,(h,k) in _exp_specs.items():
    _save_grid2(fn, [[{'expr':lambda x:2**x,'deriv':lambda x:np.log(2)*2**x,'color':'steelblue'}],
                     [{'expr':lambda x,h=h,k=k:2**(x-h)+k,'deriv':lambda x,h=h:np.log(2)*2**(x-h),'color':'steelblue'}]])

# 8.2 absolute-value pair variations.
_abs_specs={'u8_s2_p2_05_abs_pair.png':(0.5,2,-3),'u8_s2_p3_05_abs_pair.png':(-1.5,-1,2),'u8_s2_px_05_abs_pair.png':(2,3,-1)}
for fn,(a,h,k) in _abs_specs.items():
    _save_grid2(fn, [[{'expr':lambda x:np.abs(x),'deriv':lambda x:np.where(x<0,-1,1),'color':'steelblue'}],
                     [{'expr':lambda x,a=a,h=h,k=k:a*np.abs(x-h)+k,'deriv':lambda x,a=a,h=h:np.where(x<h,-a,a),'color':'steelblue'}]])

# 8.3 distinct absolute-value graph instances.
_s3_specs={
'u8_s3_p1_16_abs_graph.png':(1,-1,-4),'u8_s3_p2_06_abs_graph.png':(1,3,-2),'u8_s3_p2_16_abs_graph.png':(1,-2,-3),'u8_s3_p3_06_abs_graph.png':(2,1,-4),
'u8_s3_p3_16_abs_graph.png':(0.5,-2,-2),'u8_s3_px_06_abs_graph.png':(1,1,-5),'u8_s3_px_16_abs_graph.png':(2,-2,-6),'u8_s3_cyu_06_abs_graph.png':(1,4,-3)}
for fn,(a,h,k) in _s3_specs.items():
    _save_std(fn,[{'expr':lambda x,a=a,h=h,k=k:a*np.abs(x-h)+k,'deriv':lambda x,a=a,h=h:np.where(x<h,-a,a),'color':'steelblue'}])

# 8.4 distinct scatterplot instances.
_linear_scatter={
'u8_s4_p1_01_scatter_linear.png':([0,1,2,3,4,5],[2,5,8,11,14,17],20),
'u8_s4_p2_01_scatter_linear.png':([0,1,2,3,4,5],[4,6,8,10,12,14],18),
'u8_s4_p3_01_scatter_linear.png':([0,1,2,3,4,5],[1,4,7,10,13,16],20),
'u8_s4_px_01_scatter_linear.png':([0,1,2,3,4,5],[3,8,13,18,23,28],32)}
for fn,(xs,ys,ymax) in _linear_scatter.items(): _save_scatter(fn,xs,ys,0,6,0,ymax,True,'x','y')
_quad_scatter={
'u8_s4_p1_11_scatter_quad.png':([-3,-2,-1,0,1,2,3],[17,10,5,2,1,2,5],-4,4,0,19),
'u8_s4_p2_11_scatter_quad.png':([-3,-2,-1,0,1,2],[8,2,0,2,8,18],-4,3,0,20),
'u8_s4_p3_11_scatter_quad.png':([-3,-2,-1,0,1,2,3],[1,6,9,10,9,6,1],-4,4,0,12),
'u8_s4_px_11_scatter_quad.png':([-3,-2,-1,0,1,2,3],[1,0,1,4,9,16,25],-4,4,0,27),
'u8_s4_cyu_01_scatter_quad.png':([-2,-1,0,1,2,3,4],[10,6.5,4,2.5,2,2.5,4],-3,5,0,12),
'u8_s4_wu2_01_scatter_quad.png':([-3,-2,-1,0,1,2,3],[12,7,4,3,4,7,12],-4,4,0,14),
'u8_s4_eta_01_scatter_quad.png':([-2,-1,0,1,2,3,4],[19,9,3,1,3,9,19],-3,5,0,21)}
for fn,(xs,ys,xmin,xmax,ymin,ymax) in _quad_scatter.items(): _save_scatter(fn,xs,ys,xmin,xmax,ymin,ymax,False,'x','y')
_save_scatter('u8_s4_wu3_01_scatter_exp.png',[0,1,2,3,4],[2,4,8,16,32],0,5,0,34,False,'x','y')

# 8.5 distinct family-gallery instances.
_s5_gallery_specs=[
('u8_s5_p1_09_family_gallery.png',1,-2, -0.5,0,4, 1.6,-1, 1.0,2,-3),
('u8_s5_p2_09_family_gallery.png',-1,3, 0.5,-2,-1, 1.4,-2, 1.5,-1,0),
('u8_s5_p3_09_family_gallery.png',0.75,1, -1.0,2,5, 1.8,-3, 0.75,3,-2),
('u8_s5_px_09_family_gallery.png',-0.75,-1, 0.5,1,-4, 1.5,-1, 2.0,-2,-1)]
for fn,m,b,a,h,k,base,eshift,c,ah,ak in _s5_gallery_specs:
    _save_grid4(fn, [[{'expr':lambda x,m=m,b=b:m*x+b,'deriv':lambda x,m=m:m*np.ones_like(x),'color':'steelblue'}],
                     [{'expr':lambda x,a=a,h=h,k=k:a*(x-h)**2+k,'deriv':lambda x,a=a,h=h:2*a*(x-h),'color':'steelblue'}],
                     [{'expr':lambda x,base=base,eshift=eshift:base**(x/2)+eshift,'deriv':lambda x,base=base:(np.log(base)/2)*base**(x/2),'color':'steelblue'}],
                     [{'expr':lambda x,c=c,ah=ah,ak=ak:c*np.abs(x-ah)+ak,'deriv':lambda x,c=c,ah=ah:np.where(x<ah,-c,c),'color':'steelblue'}]])

# Secure summative equation-matched context graphs. Curves are generated over the stated domain/range.
_save_context('u8_sum_v1_q08_parabola_context.png',lambda t:-5*(t-2)**2+20,lambda t:-10*(t-2),4,22)
_save_context('u8_sum_v2_q04_parabola_context.png',lambda t:-4*(t-3)**2+36,lambda t:-8*(t-3),6,40)
_save_context('u8_sum_v3_q01_parabola_context.png',lambda t:-3*(t-2)**2+12,lambda t:-6*(t-2),4,14)
_save_context('u8_sum_v4_q07_parabola_context.png',lambda t:-2*(t-4)**2+32,lambda t:-4*(t-4),8,36)

# 3f evidence-assessment upgrade: actual Exit Ticket B graph representations
_save_std('u8_s5_etb_01_exp_graph.png',[{'expr':lambda x:4*(2**x),'deriv':lambda x:4*np.log(2)*(2**x),'color':'steelblue'}])
_save_std('u8_s5_etb_04_exp_graph.png',[{'expr':lambda x:5*(0.5**x),'deriv':lambda x:5*np.log(0.5)*(0.5**x),'color':'steelblue'}])
