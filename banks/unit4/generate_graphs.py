...
# UNIT 4 GENERATION CALLS - appended to the authoritative Framework v6.10 tool.
if __name__ == '__main__':
    def standard_file(filename, funcs, title=''):
        fig,ax=plt.subplots(figsize=(3.6,3.6)); make_standard_graph(ax,funcs,title=title); save_graph(fig,filename); plt.close(fig)
    def quad_func(a=1,h=0,k=0,label=None,color='steelblue'):
        return {'expr':lambda x,a=a,h=h,k=k:a*(x-h)**2+k,'deriv':lambda x,a=a,h=h:2*a*(x-h),'color':color,'label':label}
    def abs_func(a=1,h=0,k=0,label=None,color='steelblue'):
        return {'expr':lambda x,a=a,h=h,k=k:a*np.abs(x-h)+k,'deriv':lambda x,a=a,h=h:a*np.sign(x-h),'color':color,'label':label}
    def lin_func(m=1,b=0,label=None,color='steelblue'):
        return {'expr':lambda x,m=m,b=b:m*x+b,'deriv':lambda x,m=m:np.zeros_like(x)+m,'color':color,'label':label}
    def exp_func(A=1,r=2,h=0,k=0,label=None,color='steelblue'):
        return {'expr':lambda x,A=A,r=r,h=h,k=k:A*(r**(x-h))+k,'deriv':lambda x,A=A,r=r,h=h:A*np.log(r)*(r**(x-h)),'color':color,'label':label}
    def gallery(filename):
        fig=make_2x2_grid([[lin_func(1,1)],[quad_func(1,0,-1)],[exp_func(1,2,0,0)],[abs_func(1,0,-2)]],titles=['A','B','C','D']); save_graph(fig,filename); plt.close(fig)
    gallery('u4_s41_wtc_families.png'); gallery('u4_s41_note_gallery.png'); gallery('u4_s41_yti_gallery.png'); gallery('u4_s41_exit.png')
    for f in range(1,5): gallery(f'u4_s41_practice_f{f}.png')
    # translations: parent + moved copy
    standard_file('u4_s42_wtc_translation.png',[abs_func(1,0,0,'parent','steelblue'),abs_func(1,3,-2,'translated','firebrick')])
    standard_file('u4_s42_note_overlay.png',[quad_func(1,0,0,'parent','steelblue'),quad_func(1,2,3,'translated','firebrick')])
    standard_file('u4_s42_yti_overlay.png',[abs_func(1,0,0,'parent','steelblue'),abs_func(1,-2,-3,'translated','firebrick')])
    for f in range(1,5): standard_file(f'u4_s42_practice_f{f}.png',[quad_func(1,0,0,'parent','steelblue'),quad_func(1,f,f+1,'translated','firebrick')])
    # transformations
    standard_file('u4_s43_wtc_transform.png',[quad_func(1,0,0,'parent','steelblue'),quad_func(-2,1,3,'transformed','firebrick')])
    standard_file('u4_s43_note_compare.png',[abs_func(1,0,0,'parent','steelblue'),abs_func(2,0,0,'stretch','firebrick'),abs_func(-1,0,0,'reflect','green')])
    standard_file('u4_s43_yti_compare.png',[quad_func(1,0,0,'parent','steelblue'),quad_func(.4,0,0,'compressed','firebrick')])
    for f in range(1,5):
        a=-1 if f%2==0 else 2; h=f-2; k=f-1
        standard_file(f'u4_s43_practice_f{f}.png',[quad_func(1,0,0,'parent','steelblue'),quad_func(a,h,k,'transformed','firebrick')])
    standard_file('u4_s43_exit.png',[abs_func(1,0,0,'parent','steelblue'),abs_func(-2,0,1,'transformed','firebrick')])
    # polynomial area models
    make_rectangle_model([r'$x$',r'$3$'],[r'$x$',r'$5$'],[[r'$x^2$',r'$5x$'],[r'$3x$',r'$15$']],filename='u4_s44_wtc_area.png')
    make_rectangle_model([r'$x$',r'$2$'],[r'$x$',r'$4$'],[[r'$x^2$',r'$4x$'],[r'$2x$',r'$8$']],filename='u4_s44_note_rectangle.png')
    make_rectangle_model([r'$x$',r'$1$'],[r'$x$',r'$6$'],[[r'$x^2$',r'$6x$'],[r'$x$',r'$6$']],filename='u4_s44_yti_rectangle.png')
    for f in range(1,5):
        for k,(r,c) in enumerate([(2,f+2),(3,f+1),(f,5)],16):
            make_rectangle_model([r'$x$',rf'${r}$'],[r'$x$',rf'${c}$'],[[r'$x^2$',rf'${c}x$'],[rf'${r}x$',rf'${r*c}$']],filename=f'u4_s44_practice_f{f}_{k}.png')
    make_rectangle_model([r'$x$',r'$2$'],[r'$x$',r'$5$'],[[r'$x^2$',r'$5x$'],[r'$2x$',r'$10$']],filename='u4_s44_exit_area.png')
    # 4.5 quadratic structure graphs
    standard_file('u4_s45_wtc_forms.png',[quad_func(1,4,-4,label='quadratic')])
    standard_file('u4_s45_note_parabola.png',[quad_func(-1,1,4,label='quadratic')])
    standard_file('u4_s45_yti_parabola.png',[quad_func(1,1,-1,label='quadratic')])
    standard_file('u4_s45_exit.png',[quad_func(1,2,-4,label='quadratic')])
    for f in range(1,5):
        a=-1 if f%2==0 else 1; h=f-2; k=f-3
        standard_file(f'u4_s45_practice_f{f}.png',[quad_func(a,h,k,label='quadratic')])
    # mastery retrieval line
    standard_file('u4_mastery_s44_line.png',[lin_func(2,1,label='line')])
    # Summative line/scatter/compare figures
    specs=[(1,2),(-2,4),(2,-3),(3,1),(-1,5),(4,-2)]
    for v,(m,b) in enumerate(specs,1): standard_file(f'u4_sum_v{v}_line.png',[lin_func(m,b,label='line')])
    for v in range(1,7):
        xs=np.array([1,2,3,4,5,6,7],dtype=float); m=2+(v%3); b=8+v; noise=np.array([0,1,-1,1,0,-1,1],dtype=float); ys=m*xs+b+noise
        fig,ax=plt.subplots(figsize=(3.6,3.4)); make_scatter_plot(ax,xs,ys,0,8,0,max(ys)+8,line_of_best_fit=True,xlabel='Input',ylabel='Output',title='Linear trend'); save_graph(fig,f'u4_sum_v{v}_scatter.png'); plt.close(fig)
    for v in [5,6]: standard_file(f'u4_sum_v{v}_compare.png',[lin_func(3,2,'A','steelblue'),lin_func(1,7,'B','firebrick')])
    # Diamonds — deliberately varied blank patterns per v6.10 inventory
    make_diamond(r'$-18$',r'$-6$',r'$3$',r'$-3$',filename='diamond_41.png')
    make_diamond(r'$20$',r'$4$','',r'$9$',filename='diamond_42.png')
    make_diamond('',r'$-6$',r'$4$',r'$-2$',filename='diamond_43.png')
    make_diamond(r'$28$',r'$7$',r'$4$','',filename='diamond_44.png')
    make_diamond(r'$-21$','',r'$-3$',r'$4$',filename='diamond_45.png')
