...
# =============================================================================
# UNIT 7 APPENDED GRAPH GENERATION CALLS - representation-upgrade pass.
# Existing authoritative graph-tool code above is unchanged.
# =============================================================================

def _u7_standard(filename, a, b, color='steelblue'):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_standard_graph(ax,[{'expr':lambda x,a=a,b=b:a*(b**x),
                             'deriv':lambda x,a=a,b=b:a*(b**x)*np.log(b),
                             'color':color}],title='')
    save_graph(fig, filename); plt.close(fig)

def _u7_compare(filename, a, b1=1.5, b2=2.0):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_standard_graph(ax,[
      {'expr':lambda x,a=a,b1=b1:a*(b1**x),'deriv':lambda x,a=a,b1=b1:a*(b1**x)*np.log(b1),'color':'steelblue','label':'A'},
      {'expr':lambda x,a=a,b2=b2:a*(b2**x),'deriv':lambda x,a=a,b2=b2:a*(b2**x)*np.log(b2),'color':'firebrick','label':'B'}],title='')
    save_graph(fig, filename); plt.close(fig)

def _u7_roots(filename, r1, r2):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_standard_graph(ax,[{'expr':lambda x,r1=r1,r2=r2:(x-r1)*(x-r2),
                             'deriv':lambda x,r1=r1,r2=r2:2*x-(r1+r2),
                             'color':'steelblue'}],title='')
    save_graph(fig, filename); plt.close(fig)

def _u7_scatter(filename, ys, ymax):
    xs=[0,1,2,3,4,5]
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_scatter_plot(ax,xs,ys,xmin=0,xmax=5,ymin=0,ymax=ymax,xlabel='time',ylabel='value',title='')
    save_graph(fig, filename); plt.close(fig)

def _u7_grid(filename, m, c, h, k, qscale, ga, gb, da, db):
    fig=make_2x2_grid([
      [{'expr':lambda x,m=m,c=c:m*x+c,'deriv':lambda x,m=m:np.full_like(x,m),'color':'steelblue'}],
      [{'expr':lambda x,h=h,k=k,qscale=qscale:qscale*(x-h)**2+k,'deriv':lambda x,h=h,qscale=qscale:2*qscale*(x-h),'color':'firebrick'}],
      [{'expr':lambda x,ga=ga,gb=gb:ga*(gb**x),'deriv':lambda x,ga=ga,gb=gb:ga*(gb**x)*np.log(gb),'color':'darkorange'}],
      [{'expr':lambda x,da=da,db=db:da*(db**x),'deriv':lambda x,da=da,db=db:da*(db**x)*np.log(db),'color':'steelblue'}]],
      titles=['A','B','C','D'])
    save_graph(fig,filename); plt.close(fig)

# Section 1 - every independent classification item gets a genuinely different exponential graph.
_u7_standard('u7_s1_growth.png',2.4,1.4,'steelblue')
_u7_standard('u7_s1_decay.png',8,0.65,'firebrick')
_u7_standard('u7_s1_ps1_06_decay.png',5,0.75,'firebrick')
_u7_standard('u7_s1_ps1_16_growth.png',1.5,1.6,'steelblue')
_u7_standard('u7_s1_ps2_05_growth.png',3,1.25,'steelblue')
_u7_standard('u7_s1_ps2_15_decay.png',9,0.8,'firebrick')
_u7_standard('u7_s1_ps3_04_decay.png',4,0.6,'firebrick')
_u7_standard('u7_s1_ps3_14_growth.png',4,1.2,'steelblue')
_u7_standard('u7_s1_extra_03_growth.png',1,1.8,'steelblue')
_u7_standard('u7_s1_extra_13_decay.png',7,0.68,'firebrick')

# Section 2 - graph/equation instances. Notes Example remains the original 3(1.5)^x anchor.
_u7_standard('u7_s2_model.png',3,1.5,'steelblue')
_u7_standard('u7_s2_yti05_model.png',2,1.6,'steelblue')
_u7_standard('u7_s2_ps1_08_model.png',4,1.25,'steelblue')
_u7_standard('u7_s2_ps1_18_model.png',5,1.2,'steelblue')
_u7_standard('u7_s2_ps2_07_model.png',2.5,1.4,'steelblue')
_u7_standard('u7_s2_ps2_17_model.png',3.5,1.3,'steelblue')
_u7_standard('u7_s2_ps3_06_model.png',1.5,1.75,'steelblue')
_u7_standard('u7_s2_ps3_16_model.png',4.5,1.15,'steelblue')
_u7_standard('u7_s2_extra_05_model.png',2.25,1.8,'steelblue')
_u7_standard('u7_s2_extra_15_model.png',3.25,1.35,'steelblue')
_u7_standard('u7_s2_exit_a03_model.png',5.5,1.1,'steelblue')

# Section 3 WTC keeps one coherent shared growth/decay pair; all other independent uses are new instances.
_u7_standard('u7_s3_growth.png',2,1.5,'steelblue')
_u7_standard('u7_s3_decay.png',6,0.5,'firebrick')
_u7_standard('u7_s3_ex04_growth.png',3,1.4,'steelblue')
_u7_standard('u7_s3_yti04_decay.png',7,0.7,'firebrick')
_u7_standard('u7_s3_ps1_06_decay.png',5,0.6,'firebrick')
_u7_standard('u7_s3_ps1_16_growth.png',1.5,1.7,'steelblue')
_u7_standard('u7_s3_ps2_05_growth.png',4,1.3,'steelblue')
_u7_standard('u7_s3_ps2_15_decay.png',8,0.75,'firebrick')
_u7_standard('u7_s3_ps3_04_decay.png',4,0.8,'firebrick')
_u7_standard('u7_s3_ps3_14_growth.png',2.5,1.6,'steelblue')
_u7_standard('u7_s3_extra_03_growth.png',5,1.18,'steelblue')
_u7_standard('u7_s3_extra_13_decay.png',9,0.65,'firebrick')

# Compare-base anchor + independent instances.
_u7_compare('u7_s3_compare_bases.png',2,1.5,2.0)
_u7_compare('u7_s3_compare_bases_yti.png',2,1.25,1.75)
_u7_compare('u7_s3_ps1_08_compare.png',1.25)
_u7_compare('u7_s3_ps1_18_compare.png',2.5)
_u7_compare('u7_s3_ps2_07_compare.png',1.5)
_u7_compare('u7_s3_ps2_17_compare.png',1.0)
_u7_compare('u7_s3_ps3_06_compare.png',1.75)
_u7_compare('u7_s3_ps3_16_compare.png',2.25)
_u7_compare('u7_s3_extra_05_compare.png',2.75)
_u7_compare('u7_s3_extra_15_compare.png',3.0)
_u7_compare('u7_s3_exit_a03_compare.png',3.5)

# Mastery roots: independent graph instance per independent question.
_u7_roots('u7_s3_quadratic_roots.png',-3,2)
_u7_roots('u7_s3_ps2_24_roots.png',-4,1)
_u7_roots('u7_s3_ps3_24_roots.png',-2,3)
_u7_roots('u7_s3_extra_24_roots.png',-5,1)
_u7_roots('u7_s3_wu3_04_roots.png',-1,4)
_u7_roots('u7_s3_exit_c03_roots.png',-4,2)

# Section 4 family grids: role order remains A linear, B quadratic, C growth, D decay; geometry varies by item.
_u7_grid('u7_s4_families_grid.png',0.7,1,1,-4,0.25,2,1.5,6,0.5)
_u7_grid('u7_s4_ps1_13_grid.png',-0.6,2,-1,-3,0.3,1.5,1.6,7,0.65)
_u7_grid('u7_s4_ps2_02_grid.png',0.5,-2,2,-2,0.2,2.5,1.35,8,0.7)
_u7_grid('u7_s4_ps2_12_grid.png',-0.8,-1,0,-5,0.18,1.8,1.55,5,0.6)
_u7_grid('u7_s4_ps3_01_grid.png',1.1,0,-2,-1,0.22,3,1.3,9,0.75)
_u7_grid('u7_s4_ps3_11_grid.png',0.4,3,1,-2,0.35,1.2,1.75,6.5,0.68)
_u7_grid('u7_s4_extra_10_grid.png',-1.0,1,2,-4,0.15,2.2,1.45,7.5,0.72)
_u7_grid('u7_s4_cyu_02_grid.png',0.9,-3,-1,-2,0.28,1.6,1.65,8.5,0.62)
_u7_grid('u7_s4_wu3_01_grid.png',-0.5,0,0,-3,0.32,2.8,1.25,5.5,0.58)

# Section 5 scatter plots: each independent modeling question has its own data display.
_u7_scatter('u7_s5_scatter_growth.png',[10,15,23,33,51,75],85)
_u7_scatter('u7_s5_scatter_decay.png',[100,81,65,53,42,34],110)
_u7_scatter('u7_s5_ps1_05_growth.png',[8,12,18,27,41,61],70)
_u7_scatter('u7_s5_ps1_15_decay.png',[90,72,58,46,37,30],100)
_u7_scatter('u7_s5_ps2_04_decay.png',[80,60,46,35,26,20],90)
_u7_scatter('u7_s5_ps2_14_growth.png',[12,16,23,32,45,63],72)
_u7_scatter('u7_s5_ps3_03_growth.png',[6,10,15,24,38,60],68)
_u7_scatter('u7_s5_ps3_13_decay.png',[120,96,77,61,49,39],130)
_u7_scatter('u7_s5_extra_02_decay.png',[70,56,45,36,29,23],80)
_u7_scatter('u7_s5_extra_12_growth.png',[20,25,33,43,57,75],85)
_u7_scatter('u7_s5_cyu_03_decay.png',[110,88,70,56,45,36],120)
_u7_scatter('u7_s5_exit_a02_decay.png',[95,76,61,49,39,31],105)

# Intentional prerequisite Diamonds remain unique existing figures.
for sec,(top,bottom) in {1:(12,7),2:(18,9),3:(20,9),4:(15,8),5:(24,10)}.items():
    make_diamond(str(top),'','',''+str(bottom),f'u7_s{sec}_wu1_diamond.png')

# Secure V1-V4 already use distinct parallel graph instances; preserve them.
for v,(r1,r2) in enumerate([(-4,3),(-2,5),(-5,2),(-1,6)],1):
    _u7_roots(f'u7_sum_v{v}_roots.png',r1,r2)

# =============================================================================
# 3f EVIDENCE / ASSESSMENT UPGRADE APPEND-ONLY FIGURES
# =============================================================================

# Exit Ticket A reasonableness check: intentionally a growth graph so students
# must test the displayed representation against a decay equation.
_u7_standard('u7_s3_exit_a04_reasonable.png', 6, 1.3, 'firebrick')

def _u7_compare_quadratic_roots(filename):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_standard_graph(ax,[
      {'expr':lambda x:(x-2)*(x+6),
       'deriv':lambda x:2*x+4,
       'color':'steelblue','label':'A'},
      {'expr':lambda x:(x-2)*(x-4),
       'deriv':lambda x:2*x-6,
       'color':'firebrick','label':'B'}],title='')
    save_graph(fig, filename); plt.close(fig)

def _u7_scaled_roots(filename, r1, r2, scale):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_standard_graph(ax,[{'expr':lambda x,r1=r1,r2=r2,scale=scale:scale*(x-r1)*(x-r2),
                             'deriv':lambda x,r1=r1,r2=r2,scale=scale:scale*(2*x-(r1+r2)),
                             'color':'steelblue'}],title='')
    save_graph(fig, filename); plt.close(fig)

_u7_compare_quadratic_roots('u7_sum_v5_compare_roots.png')
_u7_scaled_roots('u7_sum_v6_intercepts.png', 1, 7, -1)
