...
# Auto-generated Unit 3 summative graph blocks appended below the required marker.

# Unit 3 summative graph-generation blocks
# All figures use graph_tool.py functions. Styling is not overridden.
# Literal filename audit: u3_summ_blank_coordinate.png u3_summ_plans_v1.png u3_summ_plans_v2.png u3_summ_plans_v3.png u3_summ_plans_v4.png u3_summ_plans_v5.png u3_summ_plans_v6.png u3_summ_plans_v7.png u3_summ_plans_v8.png u3_summ_region_v1.png u3_summ_region_v2.png u3_summ_region_v3.png u3_summ_region_v4.png

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [], title='')
save_graph(fig, 'u3_summ_blank_coordinate.png')
plt.close(fig)

_plan_defs = {
    1: (20, 8, 0, 12, 0, 10, 0, 120),
    2: (30, 5, 0, 9, 0, 10, 0, 100),
    3: (15, 6, 9, 8, 0, 8, 0, 80),
    4: (40, 3, 10, 9, 0, 10, 0, 100),
    5: (5, 2.0, 11, 1.25, 0, 12, 0, 36),
    6: (20, 8, 0, 12, 0, 8, 0, 100),
    7: (18, 7, 6, 10, 0, 8, 0, 100),
    8: (24, 4, 0, 10, 0, 8, 0, 90),
}
for _v, (_b1, _m1, _b2, _m2, _xmin, _xmax, _ymin, _ymax) in _plan_defs.items():
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, [
        {'expr': lambda x, m=_m1, b=_b1: b + m*x, 'deriv': lambda x, m=_m1: 0*x + m, 'color': 'steelblue', 'label': 'A'},
        {'expr': lambda x, m=_m2, b=_b2: b + m*x, 'deriv': lambda x, m=_m2: 0*x + m, 'color': 'firebrick', 'label': 'B'},
    ], xmin=_xmin, xmax=_xmax, ymin=_ymin, ymax=_ymax,
       xlabel='Time', ylabel='Cost', title='')
    save_graph(fig, f'u3_summ_plans_v{_v}.png')
    plt.close(fig)

_region_defs = {
    1: [(-1, 8, 'below', True), (2, -1, 'above', True)],
    2: [(-2, 10, 'below', True), (1, 1, 'above', True)],
    3: [(-1, 7, 'below', True), (1, -3, 'above', True)],
    4: [(-0.5, 6, 'below', True), (1, -2, 'above', True)],
}
for _v, _ineqs in _region_defs.items():
    _items = []
    for _idx, (_m, _b, _shade, _inclusive) in enumerate(_ineqs):
        _items.append({
            'expr': lambda x, m=_m, b=_b: m*x + b,
            'deriv': lambda x, m=_m: 0*x + m,
            'color': 'steelblue' if _idx == 0 else 'firebrick',
            'shade': _shade,
            'inclusive': _inclusive,
        })
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_inequality_graph(ax, _items, title='')
    save_graph(fig, f'u3_summ_region_v{_v}.png')
    plt.close(fig)
