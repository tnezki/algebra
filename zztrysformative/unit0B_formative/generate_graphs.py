...
# Auto-generated Unit 0B formative graph blocks appended below the uploaded graph code.

# Unit 0B DOK 1.2 intercept graphs
for filename, m, b in [
    ('u0b_dok1_2_v1_intercepts.png', 2, -4),
    ('u0b_dok1_2_v2_intercepts.png', -1, 3),
    ('u0b_dok1_2_v3_intercepts.png', 3, 6),
    ('u0b_dok1_2_v4_intercepts.png', -2, -6),
]:
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_standard_graph(ax, [{'expr': lambda x, m=m, b=b: m*x + b,
                              'deriv': lambda x, m=m: 0*x + m,
                              'color': 'steelblue'}], title='')
    save_graph(fig, filename)
    plt.close(fig)

# Unit 0B DOK 1.2 context starting-value graphs
for filename, m, b, ymax in [
    ('u0b_dok1_2_v5_context.png', 4, 12, 50),
    ('u0b_dok1_2_v6_context.png', 6, 8, 60),
]:
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, [{'expr': lambda x, m=m, b=b: m*x + b,
                             'deriv': lambda x, m=m: 0*x + m,
                             'color': 'steelblue'}],
                       0, 8, 0, ymax, xlabel='Input', ylabel='Output', title='')
    save_graph(fig, filename)
    plt.close(fig)
