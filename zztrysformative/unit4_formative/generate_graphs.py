...
# -----------------------------------------------------------------------------
# Additional Unit 4 formative graph-generation blocks appended for graph-rich forms.
# -----------------------------------------------------------------------------

_four_grid('u4_formative_s1_family_grid_v2.png', [
    [{'expr': lambda x: 0.35*(x+2)**2 - 5, 'deriv': lambda x: 0.7*(x+2), 'color': 'steelblue'}],
    [{'expr': lambda x: -0.8*x + 4, 'deriv': lambda x: -0.8*np.ones_like(x), 'color': 'steelblue'}],
    [{'expr': lambda x: np.abs(x-3) - 2, 'deriv': lambda x: np.where(x < 3, -1, 1), 'color': 'steelblue'}],
    [{'expr': lambda x: 1.35**x - 4, 'deriv': lambda x: np.log(1.35)*(1.35**x), 'color': 'steelblue'}]
], titles=[r'$A$', r'$B$', r'$C$', r'$D$'])

_four_grid('u4_formative_s1_family_grid_v3.png', [
    [{'expr': lambda x: 1.1*x - 2, 'deriv': lambda x: 1.1*np.ones_like(x), 'color': 'steelblue'}],
    [{'expr': lambda x: -0.3*(x-1)**2 + 6, 'deriv': lambda x: -0.6*(x-1), 'color': 'steelblue'}],
    [{'expr': lambda x: 0.75**x + 1, 'deriv': lambda x: np.log(0.75)*(0.75**x), 'color': 'steelblue'}],
    [{'expr': lambda x: -np.abs(x+1) + 5, 'deriv': lambda x: np.where(x < -1, 1, -1), 'color': 'steelblue'}]
], titles=[r'$A$', r'$B$', r'$C$', r'$D$'])

_four_grid('u4_formative_s1_family_grid_v4.png', [
    [{'expr': lambda x: 0.82**x + 2, 'deriv': lambda x: np.log(0.82)*(0.82**x), 'color': 'steelblue'}],
    [{'expr': lambda x: -1.2*x + 1, 'deriv': lambda x: -1.2*np.ones_like(x), 'color': 'steelblue'}],
    [{'expr': lambda x: 0.45*(x+1)**2 - 4, 'deriv': lambda x: 0.9*(x+1), 'color': 'steelblue'}],
    [{'expr': lambda x: 1.5*np.abs(x-2) - 6, 'deriv': lambda x: np.where(x < 2, -1.5, 1.5), 'color': 'steelblue'}]
], titles=[r'$A$', r'$B$', r'$C$', r'$D$'])

_two_grid('u4_formative_s1_linear_exp_v2.png', [
    [{'expr': lambda x: 0.9*x + 2, 'deriv': lambda x: 0.9*np.ones_like(x), 'color': 'steelblue'}],
    [{'expr': lambda x: 1.32**x - 1, 'deriv': lambda x: np.log(1.32)*(1.32**x), 'color': 'firebrick'}]
], titles=[r'$A$', r'$B$'])

_two_grid('u4_formative_s1_linear_exp_v3.png', [
    [{'expr': lambda x: -0.8*x + 4, 'deriv': lambda x: -0.8*np.ones_like(x), 'color': 'steelblue'}],
    [{'expr': lambda x: 0.78**x + 1, 'deriv': lambda x: np.log(0.78)*(0.78**x), 'color': 'firebrick'}]
], titles=[r'$A$', r'$B$'])

_two_grid('u4_formative_s1_abs_quad_compare_v1.png', [
    [{'expr': lambda x: np.abs(x-2)-4, 'deriv': lambda x: np.where(x < 2, -1, 1), 'color': 'steelblue'}],
    [{'expr': lambda x: 0.35*(x+1)**2-5, 'deriv': lambda x: 0.7*(x+1), 'color': 'firebrick'}]
], titles=[r'$A$', r'$B$'])

_two_grid('u4_formative_s1_abs_quad_compare_v2.png', [
    [{'expr': lambda x: -0.4*(x-2)**2+6, 'deriv': lambda x: -0.8*(x-2), 'color': 'steelblue'}],
    [{'expr': lambda x: -np.abs(x+2)+5, 'deriv': lambda x: np.where(x < -2, 1, -1), 'color': 'firebrick'}]
], titles=[r'$A$', r'$B$'])

_two_grid('u4_formative_s2_abs_translation_v2.png', [
    [{'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x < 0, -1, 1), 'color': 'steelblue'}],
    [{'expr': lambda x: np.abs(x+4) - 2, 'deriv': lambda x: np.where(x < -4, -1, 1), 'color': 'firebrick'}]
], titles=['Parent', 'Translated'])

_two_grid('u4_formative_s2_quad_translation_v2.png', [
    [{'expr': lambda x: x**2, 'deriv': lambda x: 2*x, 'color': 'steelblue'}],
    [{'expr': lambda x: (x-3)**2 + 1, 'deriv': lambda x: 2*(x-3), 'color': 'firebrick'}]
], titles=['Parent', 'Translated'])

_two_grid('u4_formative_s3_quad_transform_v2.png', [
    [{'expr': lambda x: x**2, 'deriv': lambda x: 2*x, 'color': 'steelblue'}],
    [{'expr': lambda x: (1/3)*(x+3)**2 - 2, 'deriv': lambda x: (2/3)*(x+3), 'color': 'firebrick'}]
], titles=['Parent', 'Transformed'])

_two_grid('u4_formative_s3_abs_transform_v2.png', [
    [{'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x < 0, -1, 1), 'color': 'steelblue'}],
    [{'expr': lambda x: 3*np.abs(x-2) - 5, 'deriv': lambda x: np.where(x < 2, -3, 3), 'color': 'firebrick'}]
], titles=['Parent', 'Transformed'])

_std_fig('u4_formative_s5_quadratic_feature_v2.png', [
    {'expr': lambda x: -0.5*(x+1)**2 + 4.5, 'deriv': lambda x: -(x+1), 'color': 'steelblue'}
], title='')

_two_grid('u4_formative_s5_expanded_vs_factored_v2.png', [
    [{'expr': lambda x: (x-4)*(x+2), 'deriv': lambda x: 2*x-2, 'color': 'steelblue'}],
    [{'expr': lambda x: -0.5*(x+1)**2+6, 'deriv': lambda x: -(x+1), 'color': 'firebrick'}]
], titles=[r'$A$', r'$B$'])
