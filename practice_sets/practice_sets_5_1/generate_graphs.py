...
# Consolidated bank graph-generation blocks for Unit 5 practice sets.


# Blocks from unit3_generate_graphs(4).py

# Unit 3 bank figure generation

def _std(filename, functions=None):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_standard_graph(ax, functions or [], title='')
    save_graph(fig, filename)
    plt.close(fig)

def _ineq(filename, inequalities):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_inequality_graph(ax, inequalities, title='')
    save_graph(fig, filename)
    plt.close(fig)

def _context(filename, functions, xmin, xmax, ymin, ymax, xlabel, ylabel):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, functions, xmin, xmax, ymin, ymax, xlabel=xlabel, ylabel=ylabel, title='')
    save_graph(fig, filename)
    plt.close(fig)

_std('u3_bank_standard_blank_sq_v1.png', [])
_std('u3_bank_s1_one_solution_sq_v1.png', [
    {'expr': lambda x: 0.5*x + 1, 'deriv': lambda x: np.full_like(x, 0.5), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: -1*x + 4, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'firebrick', 'label': 'B'},
])
_std('u3_bank_s1_parallel_sq_v1.png', [
    {'expr': lambda x: 2*x + 1, 'deriv': lambda x: np.full_like(x, 2.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: 2*x - 3, 'deriv': lambda x: np.full_like(x, 2.0), 'color': 'firebrick', 'label': 'B'},
])
_std('u3_bank_s1_same_line_sq_v1.png', [
    {'expr': lambda x: -2*x + 5, 'deriv': lambda x: np.full_like(x, -2.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: -2*x + 5, 'deriv': lambda x: np.full_like(x, -2.0), 'color': 'firebrick', 'label': 'B'},
])
_std('u3_bank_s2_substitution_evidence_sq_v1.png', [
    {'expr': lambda x: x + 1, 'deriv': lambda x: np.full_like(x, 1.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: -2*x + 7, 'deriv': lambda x: np.full_like(x, -2.0), 'color': 'firebrick', 'label': 'B'},
])
_context('u3_bank_s3_context_lines_sq_v1.png', [
    {'expr': lambda x: 30 + 18*x, 'deriv': lambda x: np.full_like(x, 18.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: 90 + 10*x, 'deriv': lambda x: np.full_like(x, 10.0), 'color': 'firebrick', 'label': 'B'},
], 0, 12, 0, 300, 'Months', 'Cost')
_ineq('u3_bank_s4_ineq_above_sq_v1.png', [
    {'expr': lambda x: -0.5*x + 2, 'deriv': lambda x: np.full_like(x, -0.5), 'color': 'steelblue', 'shade': 'above', 'inclusive': True},
])
_ineq('u3_bank_s4_ineq_below_dashed_sq_v1.png', [
    {'expr': lambda x: 2*x - 1, 'deriv': lambda x: np.full_like(x, 2.0), 'color': 'firebrick', 'shade': 'below', 'inclusive': False},
])
_ineq('u3_bank_s4_wrong_boundary_sq_v1.png', [
    {'expr': lambda x: -1*x + 4, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'darkorange', 'shade': 'above', 'inclusive': True},
])
_ineq('u3_bank_s5_system_region1_sq_v1.png', [
    {'expr': lambda x: x + 1, 'deriv': lambda x: np.full_like(x, 1.0), 'color': 'steelblue', 'shade': 'above', 'inclusive': True},
    {'expr': lambda x: -1*x + 7, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'firebrick', 'shade': 'below', 'inclusive': True},
])
_ineq('u3_bank_s5_system_region2_sq_v1.png', [
    {'expr': lambda x: 2*x + 3, 'deriv': lambda x: np.full_like(x, 2.0), 'color': 'steelblue', 'shade': 'below', 'inclusive': False},
    {'expr': lambda x: -1*x - 1, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'firebrick', 'shade': 'above', 'inclusive': True},
])
_ineq('u3_bank_s5_wrong_region_sq_v1.png', [
    {'expr': lambda x: -1*x + 10, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'steelblue', 'shade': 'above', 'inclusive': True},
    {'expr': lambda x: np.full_like(x, 2.0), 'deriv': lambda x: np.full_like(x, 0.0), 'color': 'firebrick', 'shade': 'below', 'inclusive': True},
])


# Blocks from unit4_generate_graphs(1).py

# Unit 4 graph-generation blocks appended below the authorized marker.

def _std_fig(filename, functions=None, title=''):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_standard_graph(ax, functions or [], title=title)
    save_graph(fig, filename)
    plt.close(fig)

def _two_grid(filename, functions_list, titles=None):
    fig = make_2x1_grid(functions_list, titles=titles)
    save_graph(fig, filename)
    plt.close(fig)

def _four_grid(filename, functions_list, titles=None):
    fig = make_2x2_grid(functions_list, titles=titles)
    save_graph(fig, filename)
    plt.close(fig)

# Shared blank coordinate planes by section.
_std_fig('u4_bank_s1_blank_standard_sq_v1.png')
_std_fig('u4_bank_s2_blank_standard_sq_v1.png')
_std_fig('u4_bank_s3_blank_standard_sq_v1.png')
_std_fig('u4_bank_s5_blank_standard_sq_v1.png')

# Section 1: function-family evidence graphs.
_four_grid('u4_bank_s1_family_grid_sq_v1.png', [
    [{'expr': lambda x: x + 1, 'deriv': lambda x: np.ones_like(x), 'color': 'steelblue'}],
    [{'expr': lambda x: 0.4*(x-1)**2 - 4, 'deriv': lambda x: 0.8*(x-1), 'color': 'steelblue'}],
    [{'expr': lambda x: np.abs(x+2) - 3, 'deriv': lambda x: np.where(x < -2, -1, 1), 'color': 'steelblue'}],
    [{'expr': lambda x: 1.4**x - 3, 'deriv': lambda x: np.log(1.4)*(1.4**x), 'color': 'steelblue'}]
], titles=[r'$A$', r'$B$', r'$C$', r'$D$'])

_two_grid('u4_bank_s1_linear_vs_exponential_sq_v1.png', [
    [{'expr': lambda x: 1.5*x + 1, 'deriv': lambda x: 1.5*np.ones_like(x), 'color': 'steelblue'}],
    [{'expr': lambda x: 1.25**x + 1, 'deriv': lambda x: np.log(1.25)*(1.25**x), 'color': 'firebrick'}]
], titles=[r'$A$', r'$B$'])

# Section 2: translations.
_two_grid('u4_bank_s2_abs_translation_sq_v1.png', [
...
], titles=['Parent', 'Translated'])

_two_grid('u4_bank_s2_quad_translation_sq_v1.png', [
    [{'expr': lambda x: x**2, 'deriv': lambda x: 2*x, 'color': 'steelblue'}],
    [{'expr': lambda x: (x+2)**2 - 5, 'deriv': lambda x: 2*(x+2), 'color': 'firebrick'}]
], titles=['Parent', 'Translated'])

# Section 3: transformations.
_two_grid('u4_bank_s3_quad_transform_sq_v1.png', [
    [{'expr': lambda x: x**2, 'deriv': lambda x: 2*x, 'color': 'steelblue'}],
    [{'expr': lambda x: -0.5*(x-1)**2 + 4, 'deriv': lambda x: -(x-1), 'color': 'firebrick'}]
], titles=['Parent', 'Transformed'])

_two_grid('u4_bank_s3_abs_reflect_stretch_sq_v1.png', [
    [{'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x < 0, -1, 1), 'color': 'steelblue'}],
    [{'expr': lambda x: -2*np.abs(x+1) + 3, 'deriv': lambda x: np.where(x < -1, 2, -2), 'color': 'firebrick'}]
], titles=['Parent', 'Transformed'])

# Section 4: polynomial structure visuals.
make_rectangle_model([r'$x$', r'$3$'], [r'$x$', r'$5$'], [[r'$x^2$', r'$5x$'], [r'$3x$', r'$15$']], 'u4_bank_s4_area_x3_x5_sq_v1.png')
make_rectangle_model([r'$x$', r'$-2$'], [r'$x$', r'$4$'], [[r'$x^2$', r'$4x$'], [r'$-2x$', r'$-8$']], 'u4_bank_s4_area_xminus2_xplus4_sq_v1.png')
make_rectangle_model([r'$2x$', r'$-1$'], [r'$x$', r'$3$'], [[r'$2x^2$', r'$6x$'], [r'$-x$', r'$-3$']], 'u4_bank_s4_area_2xminus1_xplus3_sq_v1.png')
make_rectangle_model([r'$x$', r'$4$'], [r'$x$', r'$-6$'], [['', ''], ['', '']], 'u4_bank_s4_area_blank_sq_v1.png')
make_algebra_tiles({'x2': 1, 'neg_x2': 0, 'x': 5, 'neg_x': 0, 'one': 6, 'neg_one': 0, 'y': 0, 'neg_y': 0, 'xy': 0, 'neg_xy': 0, 'y2': 0, 'neg_y2': 0}, 'u4_bank_s4_tiles_x2_5x_6_sq_v1.png')

# Section 5: quadratic feature graphs.
_std_fig('u4_bank_s5_quadratic_feature_sq_v1.png', [
    {'expr': lambda x: (x-2)**2 - 4, 'deriv': lambda x: 2*(x-2), 'color': 'steelblue'}
], title='')

_two_grid('u4_bank_s5_expanded_vs_factored_sq_v1.png', [
    [{'expr': lambda x: (x+3)*(x-1), 'deriv': lambda x: 2*x+2, 'color': 'steelblue'}],
    [{'expr': lambda x: (x-2)**2 - 9, 'deriv': lambda x: 2*(x-2), 'color': 'firebrick'}]
], titles=[r'$A$', r'$B$'])

_std_fig('u4_bank_s5_downward_quad_sq_v1.png', [
    {'expr': lambda x: -(x+1)**2 + 6, 'deriv': lambda x: -2*(x+1), 'color': 'steelblue'}
], title='')


# Blocks from unit5_generate_graphs.py
