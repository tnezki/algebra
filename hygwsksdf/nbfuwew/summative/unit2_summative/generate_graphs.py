...
# Auto-generated Unit 2 summative graph blocks appended below the required marker.

# Unit 2 summative graph-generation blocks
# All figures use graph_tool.py functions. Styling is not overridden.
# Literal filename audit: u2_summ_blank_coordinate.png u2_summ_association_v1.png u2_summ_association_v2.png u2_summ_association_v3.png u2_summ_association_v4.png u2_summ_two_plans_v7.png u2_summ_two_plans_v8.png u2_summ_scatter_compare_v7.png u2_summ_scatter_compare_v8.png

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [], title='')
save_graph(fig, 'u2_summ_blank_coordinate.png')
plt.close(fig)

_assoc_data = {
    1: ([1, 2, 3, 4, 5, 6], [5, 8, 9, 13, 15, 18]),
    2: ([1, 2, 3, 4, 5, 6], [20, 17, 16, 12, 10, 7]),
    3: ([1, 2, 3, 4, 5, 6], [8, 14, 9, 18, 13, 17]),
    4: ([1, 2, 3, 4, 5, 6], [4, 7, 11, 14, 18, 22]),
}
for _v, (_xs, _ys) in _assoc_data.items():
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_scatter_plot(ax, _xs, _ys, xmin=0, xmax=7, ymin=0, ymax=24,
                      color='steelblue', point_size=48, line_of_best_fit=False,
                      xlabel='Input', ylabel='Output', title='')
    save_graph(fig, f'u2_summ_association_v{_v}.png')
    plt.close(fig)

_plan_defs = {
    7: ((lambda x: 50 + 6*x), (lambda x: 0*x + 6), (lambda x: 12*x), (lambda x: 0*x + 12), 0, 10, 0, 120),
    8: ((lambda x: 30 + 8*x), (lambda x: 0*x + 8), (lambda x: 14*x), (lambda x: 0*x + 14), 0, 10, 0, 140),
}
for _v, (_a, _ad, _b, _bd, _xmin, _xmax, _ymin, _ymax) in _plan_defs.items():
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, [
        {'expr': _a, 'deriv': _ad, 'color': 'steelblue', 'label': 'A'},
        {'expr': _b, 'deriv': _bd, 'color': 'firebrick', 'label': 'B'},
    ], xmin=_xmin, xmax=_xmax, ymin=_ymin, ymax=_ymax,
       xlabel='Uses', ylabel='Cost', title='')
    save_graph(fig, f'u2_summ_two_plans_v{_v}.png')
    plt.close(fig)

_scatter_compare = {
    7: ([1, 2, 3, 4, 5, 6, 7, 8], [12, 18, 25, 31, 38, 45, 52, 58]),
    8: ([1, 2, 3, 4, 5, 6, 7, 8], [20, 24, 31, 35, 43, 47, 56, 61]),
}
for _v, (_xs, _ys) in _scatter_compare.items():
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_scatter_plot(ax, _xs, _ys, xmin=0, xmax=9, ymin=0, ymax=70,
                      color='steelblue', point_size=45, line_of_best_fit=True,
                      xlabel='x', ylabel='y', title='')
    save_graph(fig, f'u2_summ_scatter_compare_v{_v}.png')
    plt.close(fig)
