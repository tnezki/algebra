...
# Unit 8 assessment bank graph-generation blocks

def _std_blank(filename, title=''):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_standard_graph(ax, [], title=title)
    save_graph(fig, filename)
    plt.close(fig)

def _std_graph(filename, functions, title=''):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
...
def _context_blank(filename, xmin, xmax, ymin, ymax, xlabel, ylabel, title=''):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, [], xmin, xmax, ymin, ymax, xlabel=xlabel, ylabel=ylabel, title=title)
    save_graph(fig, filename)
    plt.close(fig)

def _scatter(filename, xs, ys, xmin, xmax, ymin, ymax, xlabel, ylabel, title='', fit=True):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_scatter_plot(ax, xs, ys, xmin, xmax, ymin, ymax, xlabel=xlabel, ylabel=ylabel, title=title, line_of_best_fit=fit)
    save_graph(fig, filename)
    plt.close(fig)

def _piecewise(filename, pieces, title=''):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_piecewise_graph(ax, pieces, title=title)
    save_graph(fig, filename)
    plt.close(fig)

_std_blank('u8_bank_standard_blank_sq_v1.png')
fig, ax = plt.subplots(figsize=(3.5, 1.25))
fig.patch.set_facecolor('white')
make_number_line_blank(ax, label=None, xmin=-10, xmax=10)
save_graph(fig, 'u8_bank_numberline_blank_sq_v1.png')
plt.close(fig)

_context_blank('u8_bank_s1_shoe_drive_blank_sq_v1.png', 0, 200, 0, 12, 'Pairs of shoes', 'Boxes', r'$20$ pairs per box')
_context_blank('u8_bank_s1_fog_rental_blank_sq_v1.png', 0, 21, 0, 360, 'Days', 'Cost (dollars)', r'Fog machine rental')
_context_blank('u8_bank_s1_parking_blank_sq_v1.png', 0, 8, 0, 6, 'Hours', 'Cost (dollars)', r'Parking cost')

lin = [{'expr': lambda x: 0.8*x+1, 'deriv': lambda x: 0*x+0.8, 'color':'steelblue', 'label':'linear'}]
quad = [{'expr': lambda x: 0.2*(x-1)**2-4, 'deriv': lambda x: 0.4*(x-1), 'color':'firebrick', 'label':'quadratic'}]
exp = [{'expr': lambda x: 2*(1.35**x)-4, 'deriv': lambda x: np.log(1.35)*2*(1.35**x), 'color':'darkorange', 'label':'exponential'}]
absf = [{'expr': lambda x: np.abs(x+2)-5, 'deriv': lambda x: np.where(x < -2, -1, 1), 'color':'steelblue', 'label':'absolute value'}]
fig = make_2x2_grid([lin, quad, exp, absf], [r'Panel A', r'Panel B', r'Panel C', r'Panel D'])
save_graph(fig, 'u8_bank_s1_family_compare_2x2_sq_v1.png')
plt.close(fig)
fig = make_2x1_grid([lin, quad], [r'Panel A', r'Panel B'])
save_graph(fig, 'u8_bank_s1_model_compare_2x1_sq_v1.png')
plt.close(fig)
fig = make_2x1_grid([exp, [{'expr': lambda x: 8*(0.75**x), 'deriv': lambda x: np.log(0.75)*8*(0.75**x), 'color':'firebrick', 'label':'decay'}]], [r'Growth', r'Decay'])
save_graph(fig, 'u8_bank_s1_growth_decay_2x1_sq_v1.png')
plt.close(fig)

parent = [{'expr': lambda x: x**2, 'deriv': lambda x: 2*x, 'color':'steelblue', 'label':'f'}]
trans = [{'expr': lambda x: (x-3)**2-2, 'deriv': lambda x: 2*(x-3), 'color':'firebrick', 'label':'g'}]
fig = make_2x1_grid([parent, trans], [r'Before', r'After'])
save_graph(fig, 'u8_bank_s2_parent_transform_2x1_sq_v1.png')
plt.close(fig)
_std_graph('u8_bank_s2_quad_transform_sq_v1.png', trans, '')
_std_graph('u8_bank_s2_abs_transform_sq_v1.png', [{'expr': lambda x: 2*np.abs(x+1)-4, 'deriv': lambda x: np.where(x < -1, -2, 2), 'color':'steelblue', 'label':'g'}], '')
_std_graph('u8_bank_s2_exp_transform_sq_v1.png', [{'expr': lambda x: 2**(x-2)+1, 'deriv': lambda x: np.log(2)*2**(x-2), 'color':'darkorange', 'label':'g'}], r'$g(x)=2^{x-2}+1$')
fig = make_2x2_grid([
    [{'expr': lambda x: (x+2)**2-3, 'deriv': lambda x: 2*(x+2), 'color':'steelblue'}],
    [{'expr': lambda x: -np.abs(x-2)+4, 'deriv': lambda x: np.where(x < 2, 1, -1), 'color':'firebrick'}],
    [{'expr': lambda x: 2*(1.25**(x+1))-5, 'deriv': lambda x: np.log(1.25)*2*(1.25**(x+1)), 'color':'darkorange'}],
    [{'expr': lambda x: -0.5*x+3, 'deriv': lambda x: 0*x-0.5, 'color':'steelblue'}]
], [r'Panel A', r'Panel B', r'Panel C', r'Panel D'])
save_graph(fig, 'u8_bank_s2_multi_transform_2x2_sq_v1.png')
plt.close(fig)

_std_graph('u8_bank_s3_abs_basic_sq_v1.png', [{'expr': lambda x: np.abs(x)-3, 'deriv': lambda x: np.where(x < 0, -1, 1), 'color':'steelblue'}], '')
_std_graph('u8_bank_s3_abs_shifted_sq_v1.png', [{'expr': lambda x: np.abs(x-2)+1, 'deriv': lambda x: np.where(x < 2, -1, 1), 'color':'steelblue'}], '')
fig = make_2x1_grid([
    [{'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x < 0, -1, 1), 'color':'steelblue'}],
    [{'expr': lambda x: np.abs(x-3)+2, 'deriv': lambda x: np.where(x < 3, -1, 1), 'color':'firebrick'}]
], [r'$y=|x|$', r'$y=|x-3|+2$'])
save_graph(fig, 'u8_bank_s3_abs_compare_2x1_sq_v1.png')
plt.close(fig)
_piecewise('u8_bank_s3_piecewise_delivery_sq_v1.png', [
    {'expr': lambda x: -x+4, 'deriv': lambda x: 0*x-1, 'domain':(-6,1), 'include_left':True, 'include_right':False, 'arrow_left':True},
    {'expr': lambda x: 0.5*x+2.5, 'deriv': lambda x: 0*x+0.5, 'domain':(1,8), 'include_left':True, 'include_right':True, 'arrow_right':True}
], r'Piecewise model')
fig, ax = plt.subplots(figsize=(3.5, 1.25))
fig.patch.set_facecolor('white')
make_number_line(ax, [{'start':-1, 'end':5, 'start_open':False, 'end_open':False, 'direction':None, 'color':'steelblue'}], xmin=-10, xmax=10)
save_graph(fig, 'u8_bank_s3_abs_numberline_sq_v1.png')
plt.close(fig)

_scatter('u8_bank_s4_cereal_scatter_sq_v1.png', [34,150,218,325,357,471], [21,198,283,567,680,1020], 0, 500, 0, 1100, r'Cardboard (in$^2$)', r'Weight (g)', r'Cereal packages', True)
_scatter('u8_bank_s4_disk_scatter_sq_v1.png', [1.3,2.4,3.7,4.7,5.3,6.3,7.7,9.0,9.6], [0.1,0.3,0.8,1.2,1.6,2.3,3.4,4.6,5.4], 0, 10, 0, 6, r'Radius (cm)', r'Weight (g)', r'Cardboard disks', True)
_scatter('u8_bank_s4_bike_scatter_sq_v1.png', [1,2,3,4,5,6,7,8,9], [5,7,10,14,17,21,25,28,32], 0, 10, 0, 36, r'Weeks', r'Miles', r'Bike training', True)
_scatter('u8_bank_s4_attendance_scatter_sq_v1.png', [71,73,78,83,91,92,73,88,95,94], [8.6,13,21.6,25.9,23.8,25.9,17.3,25.9,17.3,21.6], 65, 100, 0, 30, r'Temperature (F)', r'Attendance (thousands)', r'Amusement park', True)
_scatter('u8_bank_s4_residual_scatter_sq_v1.png', [1,2,3,4,5,6,7,8,9], [-3,-1,2,4,3,1,-1,-3,-4], 0, 10, -5, 5, r'Input', r'Residual', r'Residual pattern', False)

fig = make_2x2_grid([lin, quad, exp, absf], [r'Panel A', r'Panel B', r'Panel C', r'Panel D'])
save_graph(fig, 'u8_bank_s5_family_compare_2x2_sq_v1.png')
plt.close(fig)
_std_graph('u8_bank_s5_pair_compare_2x1_sq_v1.png', [
    {'expr': lambda x: 0.7*x+1, 'deriv': lambda x: 0*x+0.7, 'color':'steelblue', 'label':'f'},
    {'expr': lambda x: 0.15*(x-2)**2-3, 'deriv': lambda x: 0.3*(x-2), 'color':'firebrick', 'label':'g'}
], '')
_std_graph('u8_bank_s5_capstone_compare_2x1_sq_v1.png', [
    {'expr': lambda x: 0.5*x+1, 'deriv': lambda x: 0*x+0.5, 'color':'steelblue', 'label':'f'},
    {'expr': lambda x: 5*(0.8**x), 'deriv': lambda x: np.log(0.8)*5*(0.8**x), 'color':'darkorange', 'label':'g'}
], '')
_piecewise('u8_bank_s5_piecewise_capstone_sq_v1.png', [
    {'expr': lambda x: 2*x+1, 'deriv': lambda x: 0*x+2, 'domain':(-5,3), 'include_left':True, 'include_right':True, 'arrow_left':True},
    {'expr': lambda x: 0.5*x+5.5, 'deriv': lambda x: 0*x+0.5, 'domain':(3,9), 'include_left':False, 'include_right':True, 'arrow_right':True}
], r'Rate changes at $x=3$')


# Figure-fix pass addition: blank axes for 8-S4-DOK2.01 scatterplot construction task
_context_blank('u8_bank_s4_cereal_blank_sq_v1.png', 0, 500, 0, 1100, r'Cardboard (in$^2$)', r'Weight (g)', '')


# Unit 8 summative version-specific figure blocks appended below the final marker.

def _u8_fn(kind, params):
    if kind == 'linear':
        m,b=params
        return {'expr': lambda x, m=m, b=b: m*x+b, 'deriv': lambda x, m=m: 0*x+m, 'color':'steelblue', 'label':None}
    if kind == 'quad':
        a,h,k=params
        return {'expr': lambda x, a=a, h=h, k=k: a*(x-h)**2+k, 'deriv': lambda x, a=a, h=h: 2*a*(x-h), 'color':'steelblue', 'label':None}
    if kind == 'abs':
        a,h,k=params
        return {'expr': lambda x, a=a, h=h, k=k: a*np.abs(x-h)+k, 'deriv': lambda x, a=a, h=h: a*np.sign(x-h), 'color':'steelblue', 'label':None}
    if kind == 'exp':
        a,b,k=params
        return {'expr': lambda x, a=a, b=b, k=k: a*(b**x)+k, 'deriv': lambda x, a=a, b=b: a*np.log(b)*(b**x), 'color':'steelblue', 'label':None}
    raise ValueError(kind)

def _u8_save_transform(filename, parent_kind, transformed_kind, transformed_params):
    parent_params={'quad':(1,0,0),'abs':(1,0,0),'linear':(1,0),'exp':(1,2,0)}[parent_kind]
    fig = make_2x1_grid([[_u8_fn(parent_kind,parent_params)], [_u8_fn(transformed_kind, transformed_params)]], titles=['Before','After'])
    save_graph(fig, filename)
    plt.close(fig)

# Target 4: transformations across families.
_u8_save_transform('u8_sum_t4_transform_v1.png','quad','quad',(1,2,3))
_u8_save_transform('u8_sum_t4_transform_v2.png','abs','abs',(1,-3,-2))
_u8_save_transform('u8_sum_t4_transform_v3.png','quad','quad',(-1,1,4))
_u8_save_transform('u8_sum_t4_transform_v4.png','abs','abs',(2,1,-4))
_u8_save_transform('u8_sum_t4_transform_v5.png','quad','quad',(0.5,-2,1))
_u8_save_transform('u8_sum_t4_transform_v6.png','abs','abs',(-1,2,3))
_u8_save_transform('u8_sum_t4_transform_v7.png','quad','quad',(2,0,-5))
_u8_save_transform('u8_sum_t4_transform_v8.png','abs','abs',(0.5,-1,2))


def _u8_save_piecewise(filename, pieces):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_piecewise_graph(ax, pieces, title='')
    save_graph(fig, filename)
    plt.close(fig)

# Target 6: absolute value and piecewise thinking.
_u8_save_piecewise('u8_sum_t6_piece_v1.png', [
    {'expr':lambda x: -x+2, 'deriv':lambda x: 0*x-1, 'domain':(-6,1),'include_left':True,'include_right':True},
    {'expr':lambda x: x, 'deriv':lambda x: 0*x+1, 'domain':(1,6),'include_left':False,'include_right':True},
])
_u8_save_piecewise('u8_sum_t6_piece_v2.png', [
    {'expr':lambda x: x+4, 'deriv':lambda x: 0*x+1, 'domain':(-6,-1),'include_left':True,'include_right':False},
    {'expr':lambda x: -2*x+1, 'deriv':lambda x: 0*x-2, 'domain':(-1,5),'include_left':True,'include_right':True},
])
_u8_save_piecewise('u8_sum_t6_piece_v3.png', [
    {'expr':lambda x: -0.5*x+1, 'deriv':lambda x: 0*x-0.5, 'domain':(-7,0),'include_left':True,'include_right':True},
    {'expr':lambda x: 2*x+1, 'deriv':lambda x: 0*x+2, 'domain':(0,4),'include_left':False,'include_right':True},
])
_u8_save_piecewise('u8_sum_t6_piece_v4.png', [
    {'expr':lambda x: 0*x+2, 'deriv':lambda x: 0*x, 'domain':(-6,2),'include_left':True,'include_right':False},
    {'expr':lambda x: -x+4, 'deriv':lambda x: 0*x-1, 'domain':(2,7),'include_left':True,'include_right':True},
])
_u8_save_piecewise('u8_sum_t6_piece_v5.png', [
    {'expr':lambda x: x+3, 'deriv':lambda x: 0*x+1, 'domain':(-6,-2),'include_left':True,'include_right':True},
    {'expr':lambda x: -x-1, 'deriv':lambda x: 0*x-1, 'domain':(-2,5),'include_left':False,'include_right':True},
])
_u8_save_piecewise('u8_sum_t6_piece_v6.png', [
    {'expr':lambda x: -2*x-2, 'deriv':lambda x: 0*x-2, 'domain':(-5,0),'include_left':True,'include_right':False},
    {'expr':lambda x: 0.5*x-2, 'deriv':lambda x: 0*x+0.5, 'domain':(0,7),'include_left':True,'include_right':True},
])
_u8_save_piecewise('u8_sum_t6_piece_v7.png', [
    {'expr':lambda x: -x+5, 'deriv':lambda x: 0*x-1, 'domain':(-4,3),'include_left':True,'include_right':True},
    {'expr':lambda x: 2*x-4, 'deriv':lambda x: 0*x+2, 'domain':(3,7),'include_left':False,'include_right':True},
])
_u8_save_piecewise('u8_sum_t6_piece_v8.png', [
    {'expr':lambda x: 0.5*x+1, 'deriv':lambda x: 0*x+0.5, 'domain':(-8,2),'include_left':True,'include_right':False},
    {'expr':lambda x: -x+4, 'deriv':lambda x: 0*x-1, 'domain':(2,7),'include_left':True,'include_right':True},
])


def _u8_save_scatter(filename, xs, ys, xmax, ymax, fit=False):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    make_scatter_plot(ax, xs, ys, xmin=0, xmax=xmax, ymin=0, ymax=ymax, xlabel='Input', ylabel='Output', title='', line_of_best_fit=fit)
    save_graph(fig, filename)
    plt.close(fig)

# Target 7: modeling investigation with data.
_u8_save_scatter('u8_sum_t7_scatter_v1.png',[0,1,2,3,4,5],[3,8,15,24,35,48],5,55,False)
_u8_save_scatter('u8_sum_t7_scatter_v2.png',[0,1,2,3,4,5],[4,7,11,16,22,29],5,35,False)
_u8_save_scatter('u8_sum_t7_scatter_v3.png',[0,1,2,3,4,5],[10,18,31,50,80,126],5,140,False)
_u8_save_scatter('u8_sum_t7_scatter_v4.png',[0,1,2,3,4,5],[60,48,38,31,25,20],5,65,False)
_u8_save_scatter('u8_sum_t7_scatter_v5.png',[0,1,2,3,4,5],[5,12,20,27,36,43],5,50,True)
_u8_save_scatter('u8_sum_t7_scatter_v6.png',[0,1,2,3,4,5],[7,11,18,28,41,57],5,65,True)
_u8_save_scatter('u8_sum_t7_scatter_v7.png',[0,1,2,3,4,5],[20,19,22,31,44,63],5,70,False)
_u8_save_scatter('u8_sum_t7_scatter_v8.png',[0,1,2,3,4,5],[85,74,66,61,59,60],5,90,False)


def _u8_save_pair_compare(filename, funcs, labels=('A','B')):
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    fns=[]
    colors=['steelblue','firebrick']
    for (kind, params), lab, col in zip(funcs, labels, colors):
        f=_u8_fn(kind, params); f['label']=lab; f['color']=col; fns.append(f)
    make_standard_graph(ax, fns, title='')
    save_graph(fig, filename)
    plt.close(fig)

# Target 10: synthesis/capstone comparison.
_u8_save_pair_compare('u8_sum_t10_compare_v1.png', [('abs',(1,2,0)), ('linear',(1,4))])
_u8_save_pair_compare('u8_sum_t10_compare_v2.png', [('quad',(1,1,-3)), ('linear',(0,2))])
_u8_save_pair_compare('u8_sum_t10_compare_v3.png', [('quad',(1,0,-4)), ('abs',(1,0,0))])
_u8_save_pair_compare('u8_sum_t10_compare_v4.png', [('exp',(1,1.5,0)), ('linear',(1,2))])
_u8_save_pair_compare('u8_sum_t10_compare_v5.png', [('abs',(1,-1,-2)), ('linear',(-1,3))])
_u8_save_pair_compare('u8_sum_t10_compare_v6.png', [('quad',(-1,0,5)), ('linear',(0,1))])
_u8_save_pair_compare('u8_sum_t10_compare_v7.png', [('exp',(2,0.7,0)), ('linear',(-0.5,5))])
_u8_save_pair_compare('u8_sum_t10_compare_v8.png', [('quad',(0.5,2,-1)), ('abs',(1,2,-1))])
