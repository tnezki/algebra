...
# Appended Unit 2 progress tracker graph-generation blocks
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: 1.5*x - 2, 'deriv': lambda x: np.zeros_like(x)+1.5, 'color':'steelblue', 'label':None}], title='')
save_graph(fig, 'u2_pt_slope_graph_01.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [], title='')
save_graph(fig, 'u2_pt_blank_grid_01.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [{'expr': lambda x: 8*x + 12, 'deriv': lambda x: np.zeros_like(x)+8, 'color':'steelblue', 'label':None}], 0, 8, 0, 90, xlabel='Time (min)', ylabel='Distance (m)', title='')
save_graph(fig, 'u2_pt_context_distance_01.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: x + 1, 'deriv': lambda x: np.zeros_like(x)+1, 'color':'firebrick', 'label':None}], title='')
save_graph(fig, 'u2_pt_line_compare_01.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_scatter_plot(ax, [1,2,3,4,5,6,7,8], [43,46,48,53,55,59,61,64], 0, 9, 35, 70, color='steelblue', point_size=35, line_of_best_fit=True, xlabel='Week', ylabel='Height (cm)', title='')
save_graph(fig, 'u2_pt_scatter_height_01.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_scatter_plot(ax, [2,4,6,8,10,12], [20,30,39,49,68,91], 0, 14, 0, 100, color='darkorange', point_size=35, line_of_best_fit=True, xlabel='Minutes', ylabel='Cost', title='')
save_graph(fig, 'u2_pt_scatter_cost_01.png')
plt.close(fig)
