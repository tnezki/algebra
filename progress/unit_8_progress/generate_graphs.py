...
# Unit 8 progress tracker figure generation

def _pt_std(filename, functions, title=''):
...
def _pt_context(filename, functions, xmin, xmax, ymin, ymax, xlabel, ylabel, title=''):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
...
def _pt_scatter(filename, xs, ys, xmin, xmax, ymin, ymax, xlabel, ylabel, title='', fit=True):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
...
def _pt_blank_grid(filename):
    _pt_std(filename, [], '')

def _pt_func(expr, deriv, color='steelblue', label=None):
    d = {'expr': expr, 'deriv': deriv, 'color': color}
    if label is not None:
        d['label'] = label
    return d

lin = [_pt_func(lambda x: 0.8*x + 1, lambda x: 0*x + 0.8, 'steelblue')]
quad = [_pt_func(lambda x: 0.25*(x-1)**2 - 4, lambda x: 0.5*(x-1), 'firebrick')]
exp = [_pt_func(lambda x: 1.5*(1.45**x) - 4, lambda x: np.log(1.45)*1.5*(1.45**x), 'darkorange')]
absf = [_pt_func(lambda x: np.abs(x+2) - 5, lambda x: np.where(x < -2, -1, 1), 'steelblue')]
fig = make_2x2_grid([lin, quad, exp, absf], [r'Panel A', r'Panel B', r'Panel C', r'Panel D'])
save_graph(fig, 'u8_pt_family_compare_01.png')
plt.close(fig)

_pt_blank_grid('u8_pt_blank_grid_01.png')
_pt_std('u8_pt_abs_shifted_01.png', [_pt_func(lambda x: 2*np.abs(x-1)-3, lambda x: np.where(x < 1, -2, 2), 'steelblue')], '')
_pt_std('u8_pt_model_compare_01.png', [
    _pt_func(lambda x: 0.55*x+1, lambda x: 0*x+0.55, 'steelblue', 'Model A'),
    _pt_func(lambda x: 5*(0.82**x), lambda x: np.log(0.82)*5*(0.82**x), 'darkorange', 'Model B')
], '')
...
save_graph(fig, 'u8_pt_piecewise_01.png')
plt.close(fig)

_pt_scatter('u8_pt_scatter_linear_01.png', [1,2,3,4,5,6,7,8,9], [4,7,9,14,17,21,22,27,31], 0, 10, 0, 36, r'Input', r'Output', '', True)
_pt_scatter('u8_pt_scatter_blank_01.png', [], [], 0, 500, 0, 1000, r'Cardboard area', r'Weight', '', False)
