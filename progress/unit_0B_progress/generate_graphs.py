...
# Auto-generated Unit 0B progress tracker graph blocks appended below the required marker.

# u0b_pt_blank_coordinate.png
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [], title='')
save_graph(fig, 'u0b_pt_blank_coordinate.png')
plt.close(fig)

# u0b_pt_intercepts_01.png
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: 2*x + 6, 'deriv': lambda x: 0*x + 2, 'color': 'steelblue'}], title='')
save_graph(fig, 'u0b_pt_intercepts_01.png')
plt.close(fig)

# u0b_pt_line_equation_01.png
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: 2*x + 3, 'deriv': lambda x: 0*x + 2, 'color': 'steelblue'}], title='')
save_graph(fig, 'u0b_pt_line_equation_01.png')
plt.close(fig)

# u0b_pt_compare_two_lines_01.png
fig = make_2x1_grid([
    [{'expr': lambda x: 1*x + 8, 'deriv': lambda x: 0*x + 1, 'color': 'steelblue'}],
    [{'expr': lambda x: 3*x + 0, 'deriv': lambda x: 0*x + 3, 'color': 'firebrick'}]
], titles=[r'Graph A', r'Graph B'])
save_graph(fig, 'u0b_pt_compare_two_lines_01.png')
plt.close(fig)

# u0b_pt_claim_intercepts_01.png
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: -2*x + 8, 'deriv': lambda x: 0*x - 2, 'color': 'firebrick'}], title='')
save_graph(fig, 'u0b_pt_claim_intercepts_01.png')
plt.close(fig)

# u0b_pt_graph_table_compare_01.png
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: 2*x + 3, 'deriv': lambda x: 0*x + 2, 'color': 'steelblue'}], title='')
save_graph(fig, 'u0b_pt_graph_table_compare_01.png')
plt.close(fig)
