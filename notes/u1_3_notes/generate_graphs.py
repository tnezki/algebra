...
# NOTES-SPECIFIC APPEND BLOCK — Unit 1 Section 3
if __name__ == '__main__':
    fl=[
      [{'expr':lambda x:x,'deriv':lambda x:np.ones_like(x),'color':'steelblue','label':None}],
      [{'expr':lambda x:0.25*x**2-2,'deriv':lambda x:0.5*x,'color':'steelblue','label':None}],
      [{'expr':lambda x:2**(x/2)-1,'deriv':lambda x:(np.log(2)/2)*2**(x/2),'color':'steelblue','label':None}],
      [{'expr':lambda x:np.abs(x)-2,'deriv':lambda x:np.where(x<0,-1.0,1.0),'color':'steelblue','label':None}]
    ]
    fig=make_2x2_grid(fl,[r'Linear',r'Quadratic',r'Exponential',r'Absolute Value'])
    save_graph(fig,'u1_s3_notes_families.png')
    plt.close(fig)
