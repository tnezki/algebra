...
# Appended Unit 1 notes graph-generation blocks — Section 1.3
fig = make_2x2_grid([
    [{'expr': lambda x: x + 1, 'deriv': lambda x: 0*x + 1, 'color':'steelblue', 'label':None}],
    [{'expr': lambda x: x**2 - 4, 'deriv': lambda x: 2*x, 'color':'steelblue', 'label':None}],
    [{'expr': lambda x: 2**x, 'deriv': lambda x: np.log(2)*2**x, 'color':'steelblue', 'label':None}],
...
save_graph(fig, 'u1_notes_s3_family_grid.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [
    {'expr': lambda x: 10 + 6*x, 'deriv': lambda x: 0*x + 6, 'color':'steelblue', 'label':'A'},
    {'expr': lambda x: 10*(1.5**x), 'deriv': lambda x: 10*np.log(1.5)*(1.5**x), 'color':'firebrick', 'label':'B'},
], xmin=0, xmax=5, ymin=0, ymax=80, xlabel='Meeting', ylabel='Members', title='')
save_graph(fig, 'u1_notes_s3_linear_exp.png')
plt.close(fig)
