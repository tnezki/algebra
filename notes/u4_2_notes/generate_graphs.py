...
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x < 0, -1, 1), 'color': 'steelblue', 'label': 'Parent'}, {'expr': lambda x: np.abs(x-3)+2, 'deriv': lambda x: np.where(x < 3, -1, 1), 'color': 'firebrick', 'label': 'Translated'}], title='')
for _x, _y in [(0, 0), (3, 2), (2, 3), (4, 3)]:
    ax.plot(_x, _y, 'o', markersize=7, markerfacecolor='white', markeredgecolor='firebrick', markeredgewidth=2, zorder=6)
save_graph(fig, 'u4_s2_mn_abs_translation.png')
plt.close(fig)

_std_fig('u4_s2_blank_notes.png')


fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: x**2, 'deriv': lambda x: 2*x, 'color': 'steelblue', 'label': 'Parent'}, {'expr': lambda x: (x+2)**2-4, 'deriv': lambda x: 2*(x+2), 'color': 'firebrick', 'label': 'Translated'}], title='')
for _x, _y in [(0, 0), (-2, -4), (-1, -3), (-3, -3)]:
    ax.plot(_x, _y, 'o', markersize=7, markerfacecolor='white', markeredgecolor='firebrick', markeredgewidth=2, zorder=6)
save_graph(fig, 'u4_s2_mn_horizontal_vertical.png')
plt.close(fig)
