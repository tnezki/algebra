...
# Unit 6 notes figure generation helpers

def _std(fn, filename, points=None):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_standard_graph(ax, fn, title='')
    if points:
        ax.scatter([p[0] for p in points], [p[1] for p in points], color='firebrick', s=40, zorder=5)
    save_graph(fig, filename)
    plt.close(fig)

def _ctx(fn, filename, points=None, xmax=10, ymax=80, xlabel='x', ylabel='y'):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, fn, 0, xmax, 0, ymax, xlabel=xlabel, ylabel=ylabel, title='')
    if points:
        ax.scatter([p[0] for p in points], [p[1] for p in points], color='firebrick', s=40, zorder=5)
    save_graph(fig, filename)
    plt.close(fig)
_ctx([{'expr':lambda x: -4*(x-5)**2+60, 'deriv':lambda x: -8*(x-5), 'color':'steelblue', 'label':None}], 'u6_bank_s5_dok4_1_sq_v1.png', xmax=10, ymax=70, xlabel='Time', ylabel='Height')

_ctx([{'expr':lambda x: -4*(x-2)**2+16, 'deriv':lambda x: -8*(x-2), 'color':'steelblue', 'label':None}], 'u6_s5_mn1_features.png', [(0,0),(2,16),(4,0)], xmax=5, ymax=18, xlabel='Time', ylabel='Height')
_ctx([{'expr':lambda x: -16*x**2+48*x+4, 'deriv':lambda x: -32*x+48, 'color':'steelblue', 'label':None}], 'u6_s5_mn2_ground.png', [(3.081,0)], xmax=4, ymax=45, xlabel='Time', ylabel='Height')
_ctx([{'expr':lambda x: x*(30-2*x), 'deriv':lambda x: 30-4*x, 'color':'steelblue', 'label':None}], 'u6_s5_mn3_area_opt.png', [(7.5,112.5)], xmax=15, ymax=120, xlabel='Width', ylabel='Area')
