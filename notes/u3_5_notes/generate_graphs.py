...
def _notes_standard(filename, functions=None, points=None):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_standard_graph(ax, functions or [], title='')
    if points:
        for (px, py, label) in points:
            ax.plot(px, py, 'o', markersize=6, markerfacecolor='white', markeredgecolor='#222222', markeredgewidth=1.5, zorder=10)
            if label:
                ax.text(px + 0.35, py + 0.35, label, fontfamily='Times New Roman', fontsize=10, fontweight='bold', zorder=11)
    save_graph(fig, filename)
    plt.close(fig)

def _notes_context(filename, functions, xmin, xmax, ymin, ymax, xlabel, ylabel, points=None):
    ...
    if points:
        for (px, py, label) in points:
            ax.plot(px, py, 'o', markersize=6, markerfacecolor='white', markeredgecolor='#222222', markeredgewidth=1.5, zorder=10)
            if label:
                ax.text(px + (xmax-xmin)*0.03, py + (ymax-ymin)*0.03, label, fontfamily='Times New Roman', fontsize=10, fontweight='bold', zorder=11)
    save_graph(fig, filename)
    plt.close(fig)

def _notes_ineq(filename, inequalities, points=None):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
...
_notes_standard('u3_bank_standard_blank_sq_v1.png', [])
_notes_ineq('u3_s5_mn1_overlap.png', [
    {'expr': lambda x: x + 1, 'deriv': lambda x: np.full_like(x, 1.0), 'color': 'steelblue', 'shade': 'above', 'inclusive': True},
    {'expr': lambda x: -x + 7, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'firebrick', 'shade': 'below', 'inclusive': True},
], [(2, 4, '(2,4)')])
_notes_ineq('u3_s5_mn2_boundaries.png', [
    {'expr': lambda x: 2*x + 3, 'deriv': lambda x: np.full_like(x, 2.0), 'color': 'steelblue', 'shade': 'below', 'inclusive': False},
    {'expr': lambda x: -x - 1, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'firebrick', 'shade': 'above', 'inclusive': True},
])
_notes_ineq('u3_s5_mn4_point_test.png', [
    {'expr': lambda x: -x + 8, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'steelblue', 'shade': 'below', 'inclusive': True},
    {'expr': lambda x: 2*x - 1, 'deriv': lambda x: np.full_like(x, 2.0), 'color': 'firebrick', 'shade': 'above', 'inclusive': True},
], [(2, 4, '(2,4)'), (4, 3, '(4,3)')])
