...
# Unit 7 notes graph generation blocks
# Appended below the final marker; graph_tool source above is unchanged.


def _exp(a,b,k=0,color='steelblue',label=None):
    return {'expr': lambda x: a*(b**x)+k, 'deriv': lambda x: a*np.log(b)*(b**x), 'color': color, 'label': label}
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_standard_graph(ax, [_exp(2,2,0,'steelblue','function')], title='')
ax.scatter([0],[2], color='firebrick', s=36, zorder=5)
save_graph(fig, 'u7_s3_key_points.png'); plt.close(fig)
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_standard_graph(ax, [_exp(3,2,1,'steelblue','shifted')], title='')
ax.axhline(1, color='firebrick', linewidth=1.4, linestyle='--')
ax.scatter([0],[4], color='firebrick', s=36, zorder=5)
save_graph(fig, 'u7_s3_shifted_asymptote.png'); plt.close(fig)
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_standard_graph(ax, [], title='')
save_graph(fig, 'u7_s3_blank_coordinate.png'); save_graph(fig, 'u7_bank_blank_coordinate_sq_v1.png'); plt.close(fig)
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_standard_graph(ax, [_exp(5,0.5,0,'steelblue','graph')], title='')
ax.scatter([0,1,2,3],[5,2.5,1.25,0.625], color='firebrick', s=28, zorder=5)
save_graph(fig, 'u7_s3_mismatch_decay.png'); plt.close(fig)
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_standard_graph(ax, [_exp(3,2,0,'steelblue','graph')], title='')
ax.scatter([0,1,2,3],[3,6,12,24], color='firebrick', s=28, zorder=5)
save_graph(fig, 'u7_s3_mismatch_growth.png'); plt.close(fig)
