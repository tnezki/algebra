...
# Appended Unit 2 notes graph-generation blocks — Section 2.2
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: -3*x + 4, 'deriv': lambda x: np.zeros_like(x)-3, 'color':'steelblue', 'label':None}], title='')
ax.scatter([0,1], [4,1], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u2_notes_s2_slope_intercept.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: (2/3)*x - 3, 'deriv': lambda x: np.zeros_like(x)+(2/3), 'color':'steelblue', 'label':None}], title='')
ax.scatter([0,3], [-3,-1], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u2_notes_s2_graph_line.png')
plt.close(fig)

fig = make_2x2_grid([
    [{'expr': lambda x: x+4, 'deriv': lambda x: np.zeros_like(x)+1, 'color':'steelblue', 'label':None}],
    [{'expr': lambda x: x-3, 'deriv': lambda x: np.zeros_like(x)+1, 'color':'firebrick', 'label':None}],
    [{'expr': lambda x: -x+2, 'deriv': lambda x: np.zeros_like(x)-1, 'color':'darkorange', 'label':None}],
    [{'expr': lambda x: 2*x+1, 'deriv': lambda x: np.zeros_like(x)+2, 'color':'steelblue', 'label':None}],
], titles=['','','',''])
save_graph(fig, 'u2_notes_s2_four_lines.png')
plt.close(fig)
