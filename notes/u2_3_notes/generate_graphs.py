...
# Appended Unit 2 notes graph-generation blocks — Section 2.3
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: 5*x - 7, 'deriv': lambda x: np.zeros_like(x)+5, 'color':'steelblue', 'label':None}], title='')
ax.scatter([0,1], [-7,-2], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u2_notes_s3_slope_intercept.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: -2*x + 6, 'deriv': lambda x: np.zeros_like(x)-2, 'color':'steelblue', 'label':None}], title='')
ax.scatter([0,1], [6,4], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u2_notes_s3_from_graph.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: 3*x + 4, 'deriv': lambda x: np.zeros_like(x)+3, 'color':'steelblue', 'label':None}], title='')
ax.scatter([0,1,2], [4,7,10], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u2_notes_s3_equiv.png')
plt.close(fig)
