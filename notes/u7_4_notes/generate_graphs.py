...
def _exp(a,b,color='steelblue',label=None):
    return {'expr': lambda x: a*(b**x), 'deriv': lambda x: a*np.log(b)*(b**x), 'color': color, 'label': label}
def _line(m,b,color='firebrick',label=None):
    return {'expr': lambda x: m*x+b, 'deriv': lambda x: 0*x+m, 'color': color, 'label': label}
def _quad(a,h,k,color='darkorange',label=None):
    return {'expr': lambda x: a*((x-h)**2)+k, 'deriv': lambda x: 2*a*(x-h), 'color': color, 'label': label}
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_standard_graph(ax, [_line(1,1,'firebrick','linear'), _quad(0.35,0,0,'darkorange','quadratic'), _exp(1,1.7,'steelblue','exponential')], title='')
save_graph(fig, 'u7_s4_family_shapes.png'); plt.close(fig)
fig, ax = plt.subplots(figsize=(3.5,3.5))
make_context_graph(ax, [_line(4,12,'firebrick','linear'), _exp(12,1.3,'steelblue','exponential')], 0, 6, 0, 70, xlabel='Step', ylabel='Value', title='')
save_graph(fig, 'u7_s4_linear_exp_compare.png'); plt.close(fig)
