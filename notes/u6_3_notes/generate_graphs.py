...
def _ctx(fn, filename, points=None, xmax=10, ymax=80, xlabel='x', ylabel='y'):
    ...
    save_graph(fig, filename)
    plt.close(fig)
_std([{'expr':lambda x: x**2-9*x+18, 'deriv':lambda x: 2*x-9, 'color':'steelblue', 'label':None}], 'u6_bank_s3_dok4_1_sq_v1.png')

_std([{'expr':lambda x: 2*x**2-5*x-3, 'deriv':lambda x: 4*x-5, 'color':'steelblue', 'label':None}], 'u6_s3_mn1_abc_graph.png', [(-0.5,0),(3,0)])
_std([{'expr':lambda x: x**2-4*x-6, 'deriv':lambda x: 2*x-4, 'color':'steelblue', 'label':None}], 'u6_s3_mn2_formula_roots.png', [(2-np.sqrt(10),0),(2+np.sqrt(10),0)])
fig, axes = plt.subplots(1, 3, figsize=(9, 3))
fig.patch.set_facecolor('white')
for ax, fn, pts in zip(axes, [
    [{'expr':lambda x: x**2-4, 'deriv':lambda x: 2*x, 'color':'steelblue', 'label':None}],
    [{'expr':lambda x: (x-1)**2, 'deriv':lambda x: 2*(x-1), 'color':'steelblue', 'label':None}],
    [{'expr':lambda x: x**2+4, 'deriv':lambda x: 2*x, 'color':'steelblue', 'label':None}]
], [[(-2,0),(2,0)], [(1,0)], []]):
    make_standard_graph(ax, fn, title='')
    if pts:
        ax.scatter([p[0] for p in pts], [p[1] for p in pts], color='firebrick', s=40, zorder=5)
save_graph(fig, 'u6_s3_mn3_discriminant.png')
plt.close(fig)
