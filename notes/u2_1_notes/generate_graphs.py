...
# Appended Unit 2 notes graph-generation blocks — Section 2.1
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: -0.75*x + 2, 'deriv': lambda x: np.zeros_like(x)-0.75, 'color':'steelblue', 'label':None}], title='')
ax.scatter([-4,4], [5,-1], color='firebrick', s=40, zorder=5)
save_graph(fig, 'u2_notes_s1_slope_points.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [{'expr': lambda x: 2*x + 5, 'deriv': lambda x: np.zeros_like(x)+2, 'color':'steelblue', 'label':None}], 0, 5, 0, 16, xlabel='Items', ylabel='Cost', title='')
ax.scatter([0,1,2,3], [5,7,9,11], color='firebrick', s=30, zorder=5)
save_graph(fig, 'u2_notes_s1_rate_context.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: 1.5*x + 4, 'deriv': lambda x: np.zeros_like(x)+1.5, 'color':'steelblue', 'label':None}], title='')
ax.scatter([-2,4,8], [1,10,16], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u2_notes_s1_compare_slopes.png')
plt.close(fig)
