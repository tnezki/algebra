...
# Appended Unit 1 notes graph-generation blocks — Section 1.1
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [{'expr': lambda x: 2*x + 6, 'deriv': lambda x: 0*x + 2, 'color':'steelblue', 'label':None}], xmin=0, xmax=5, ymin=0, ymax=16, xlabel='Notebooks', ylabel='Cost', title='')
ax.scatter([0,1,2,3], [6,8,10,12], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u1_notes_s1_table_graph.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [{'expr': lambda x: 3*x + 4, 'deriv': lambda x: 0*x + 3, 'color':'steelblue', 'label':None}], xmin=0, xmax=5, ymin=0, ymax=22, xlabel='Input', ylabel='Output', title='')
ax.scatter([0,1,2,3], [4,7,10,13], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u1_notes_s1_rule_graph.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [{'expr': lambda x: 4*x + 5, 'deriv': lambda x: 0*x + 4, 'color':'steelblue', 'label':None}], xmin=0, xmax=5, ymin=0, ymax=28, xlabel='Input', ylabel='Output', title='')
ax.scatter([0,1,2,3], [5,9,13,17], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u1_notes_s1_compare_graph.png')
plt.close(fig)
