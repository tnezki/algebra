...
# NOTES-SPECIFIC APPEND BLOCK — Unit 1 Section 1
if __name__ == '__main__':
    fig, ax = plt.subplots(figsize=(3.5,3.5))
    functions=[{'expr':lambda x:2*x+1,'deriv':lambda x:np.full_like(x,2.0),'color':'steelblue','label':None}]
    make_standard_graph(ax, functions, title=r'$y=2x+1$')
    save_graph(fig,'u1_s1_notes_bridge.png')
    plt.close(fig)
