...
# Appended Unit 2 notes graph-generation blocks — Section 2.4
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [
    {'expr': lambda x: -6*x, 'deriv': lambda x: np.zeros_like(x)-6, 'color':'steelblue', 'label':'A'},
    {'expr': lambda x: 4*x, 'deriv': lambda x: np.zeros_like(x)+4, 'color':'firebrick', 'label':'B'},
], title='')
save_graph(fig, 'u2_notes_s4_magnitude.png')
plt.close(fig)
...
], 0, 8, 0, 60, xlabel='Uses', ylabel='Cost', title='')
save_graph(fig, 'u2_notes_s4_plan_compare.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: 2*x + 1, 'deriv': lambda x: np.zeros_like(x)+2, 'color':'steelblue', 'label':None}], title='')
ax.scatter([0,2,4], [1,5,9], color='firebrick', s=35, zorder=5)
save_graph(fig, 'u2_notes_s4_graph_table.png')
plt.close(fig)
