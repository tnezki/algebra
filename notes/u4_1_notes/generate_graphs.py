...
_four_grid('u4_s1_mn_family_shapes.png', [
    [{'expr': lambda x: 2*x - 1, 'deriv': lambda x: 2*np.ones_like(x), 'color': 'steelblue'}],
    [{'expr': lambda x: 0.5*(x-1)**2 - 3, 'deriv': lambda x: (x-1), 'color': 'steelblue'}],
    [{'expr': lambda x: 1.6**x - 2, 'deriv': lambda x: np.log(1.6)*(1.6**x), 'color': 'steelblue'}],
...
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: x**2 + 2*x + 1, 'deriv': lambda x: 2*x + 2, 'color': 'steelblue'}], title='')
for _x, _y in [(0, 1), (1, 4), (2, 9), (3, 16)]:
    ax.plot(_x, _y, 'o', markersize=7, markerfacecolor='white', markeredgecolor='firebrick', markeredgewidth=2, zorder=6)
save_graph(fig, 'u4_s1_mn_quadratic_differences.png')
plt.close(fig)
