...
# Appended Unit 1 notes graph-generation blocks — Section 1.2
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: 2*x + 7, 'deriv': lambda x: 0*x + 2, 'color':'steelblue', 'label':None}], title='')
ax.scatter([3], [13], color='firebrick', s=45, zorder=5)
save_graph(fig, 'u1_notes_s2_function_eval.png')
plt.close(fig)

fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_scatter_plot(ax, [1,1,2,3], [4,7,9,11], xmin=0, xmax=4, ymin=0, ymax=12, color='steelblue', point_size=45, line_of_best_fit=False, xlabel='Input', ylabel='Output', title='')
save_graph(fig, 'u1_notes_s2_relation_test.png')
plt.close(fig)
