...
def _draw_axes_standard(ax, xmin, xmax, ymin, ymax):
    ...
    tri = dict(arrowstyle='-|>', color='black', lw=2.5, mutation_scale=16)
...
def make_standard_graph(ax, functions, title=''):
    ...
    ax.grid(True, which='minor', color='#888888', linewidth=0.6)
    ...
    ax.tick_params(which='major', length=5, width=1.2, color='black')
...
def make_2x2_grid(functions_list, titles=None):
    ...
    for ax, functions, title in zip(axes.flatten(), functions_list, titles):
        ...
        ax.set_xticks(np.arange(XMIN, XMAX+1, 1), minor=True)
        ax.set_yticks(np.arange(YMIN, YMAX+1, 1), minor=True)
        ax.grid(True, which='minor', color='#888888', linewidth=0.4)

        ax.set_xticks([-10, -5, 5, 10])
        ax.set_yticks([-10, -5, 5, 10])
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        ax.grid(True, which='major', color='#888888', linewidth=0.4)
        ax.tick_params(which='major', length=3.5, width=0.9, color='black')
        ...
        tri = dict(arrowstyle='-|>', color='black', lw=1.4, mutation_scale=8)
...
# Standard -10 to 10 plane. Each piece has its own domain and color.
...
def _pw_dot(ax, x, y, open_dot, color, size=7):
    if open_dot:
        ax.plot(x, y, 'o', markersize=size, markerfacecolor='white',
                markeredgecolor=color, markeredgewidth=2, zorder=6,
                clip_on=False)
...
def make_piecewise_graph(ax, pieces, title=''):
    XMIN, XMAX, YMIN, YMAX = -10, 10, -10, 10
    ...
    for piece in pieces:
        ...
        if not arrow_left and YMIN <= ya <= YMAX:
            _pw_dot(ax, a, ya, open_dot=not inc_left, color=color)

        yb = f(b)
        if not arrow_right and YMIN <= yb <= YMAX:
            _pw_dot(ax, b, yb, open_dot=not inc_right, color=color)
...
def make_unit_circle_angles(ax_angle_deg=None, show_coord=True,
    ...
    if ax_angle_deg is not None:
        ...
        ax.plot([0, px], [0, py], color=blue, linewidth=2.5, zorder=4)
...
def make_trig_graph(ax, functions, title=''):
    ...
    ax.grid(True, color='#888888', linewidth=0.6, zorder=0)
    ax.tick_params(which='major', length=4, width=1.0, color='black')
    ...
    tri = dict(arrowstyle='-|>', color='black', lw=2.0, mutation_scale=14)
...
def make_bar_chart(ax, categories, values, title='',
    ...
    ax.yaxis.grid(True, color='#888888', linewidth=0.6, zorder=0)
...
def make_scatter_plot(ax, x_data, y_data,
    ...
    ax.scatter(x_data, y_data, color=color, s=point_size,
    ...
    ax.set_xlim(xmin - mx*0.2, xmax + mx)
    ax.set_ylim(ymin - my*0.2, ymax + my)
    ...
    for xt in x_ticks:
        ax.axvline(xt, color='#888888', linewidth=0.6, zorder=0)
    for yt in y_ticks:
        ax.axhline(yt, color='#888888', linewidth=0.6, zorder=0)
    ...
    tri = dict(arrowstyle='-|>', color='black', lw=2.5, mutation_scale=16)
    ax.annotate('', xy=(xmax + mx, ymin), xytext=(xmax + mx*0.3, ymin),
                arrowprops=tri)
    ax.annotate('', xy=(xmin, ymax + my), xytext=(xmin, ymax + my*0.3),
                arrowprops=tri)
...
# ─────────────────────────────────────────────────────────────────────────────
# GENERATED GRAPH BLOCKS FOR u1_2_notes
# ─────────────────────────────────────────────────────────────────────────────
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
...
# Match the requested portable output directory for this package.
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'figures')
os.makedirs(OUTPUT_DIR, exist_ok=True)

def save_graph(fig, filename):
    """Save generated curriculum figures to ./figures at the intended canvas size."""
    path = os.path.join(OUTPUT_DIR, filename)
    fig.savefig(path, dpi=144)
    print(f'Saved: {path}')

# ── GRAPH 1 ──────────────────────────────────────────────────
# u1_2_function_machine_sq_v2.png | custom graph_tool-backed diagram | function machine
# design notes: conceptual machine diagram only; no answer-revealing values or task-specific computation.
...
ax.axis('off')
box_specs = [
    (0.7, 4.2, 2.2, 1.6, 'Input'),
    (3.9, 4.2, 2.2, 1.6, 'Rule'),
    (7.1, 4.2, 2.2, 1.6, 'Output'),
]
for x, y, w, h, label in box_specs:
    rect = mpatches.FancyBboxPatch(
        (x, y), w, h,
        boxstyle='round,pad=0.08,rounding_size=0.18',
        linewidth=1.8, edgecolor='#222222', facecolor='white'
    )
    ax.add_patch(rect)
    ax.text(x + w/2, y + h/2, label,
            ha='center', va='center',
            fontfamily='Times New Roman', fontsize=15, fontweight='bold')
arrow = dict(arrowstyle='-|>', color='#222222', lw=1.8, mutation_scale=16)
ax.annotate('', xy=(3.75, 5.0), xytext=(3.05, 5.0), arrowprops=arrow)
ax.annotate('', xy=(6.95, 5.0), xytext=(6.25, 5.0), arrowprops=arrow)
ax.text(5.0, 6.65, 'One input goes in.',
        ha='center', va='center', fontfamily='Times New Roman', fontsize=12)
ax.text(5.0, 3.25, 'One output comes out.',
        ha='center', va='center', fontfamily='Times New Roman', fontsize=12)
save_graph(fig, 'u1_2_function_machine_sq_v2.png')
plt.close(fig)

# ── GRAPH 2 ──────────────────────────────────────────────────
# u1_2_relation_repeated_outputs_sq_v3.png | make_standard_graph + scatter | Relation A repeated outputs
# design notes: four unlabeled integer-grid points; y=4 repeats, but no input repeats. No coordinate labels on points.
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_standard_graph(ax, [], title='Relation A')
ax.scatter([-3, 0, 2, 4], [4, 4, 1, 6], color='steelblue', s=46, zorder=5)
fig.tight_layout(pad=0.8)
save_graph(fig, 'u1_2_relation_repeated_outputs_sq_v3.png')
plt.close(fig)

# ── GRAPH 3 ──────────────────────────────────────────────────
# u1_2_relation_repeated_input_sq_v3.png | make_standard_graph + scatter | Relation B repeated input
# design notes: four unlabeled integer-grid points; x=1 appears with two different outputs. No coordinate labels on points.
fig, ax = plt.subplots(figsize=(3.5, 3.5))
make_standard_graph(ax, [], title='Relation B')
ax.scatter([-2, 1, 1, 4], [1, 3, -2, 5], color='steelblue', s=46, zorder=5)
fig.tight_layout(pad=0.8)
