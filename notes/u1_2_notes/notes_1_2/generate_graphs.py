...
# ── 1: Relation A — function, output 4 repeats for inputs 0 and 2 ────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_scatter_plot(ax,
    x_data=[0, 1, 2, 3], y_data=[4, 3, 4, 5],
    xmin=0, xmax=4, ymin=0, ymax=7,
    xlabel='Input', ylabel='Output', title='Relation A')
save_graph(fig, 'u1_2_relation_repeated_outputs_sq_v3.png')
plt.close(fig)

# ── 2: Relation B — NOT a function, input 2 maps to both 5 and 2 ─────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_scatter_plot(ax,
    x_data=[0, 1, 2, 2], y_data=[1, 3, 5, 2],
    xmin=0, xmax=4, ymin=0, ymax=7,
    xlabel='Input', ylabel='Output', title='Relation B')
save_graph(fig, 'u1_2_relation_repeated_input_sq_v3.png')
plt.close(fig)

# ── 3: Math Notes 3 — Allowed A: different inputs, same output ───────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_scatter_plot(ax,
    x_data=[1, 2, 3], y_data=[4, 4, 6],
    xmin=0, xmax=5, ymin=0, ymax=8,
    xlabel='Input', ylabel='Output', title='Allowed')
save_graph(fig, 'u1_2_mn3_allowed_A.png')
plt.close(fig)

# ── 4: Math Notes 3 — Not Allowed A: input 1 maps to 3 and 5 ────────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_scatter_plot(ax,
    x_data=[1, 1, 2], y_data=[3, 5, 6],
    xmin=0, xmax=5, ymin=0, ymax=8,
    color='firebrick',
    xlabel='Input', ylabel='Output', title='Not Allowed')
save_graph(fig, 'u1_2_mn3_not_allowed_A.png')
plt.close(fig)

# ── 5: Math Notes 3 — Allowed B: inputs 2 and 4 both map to 3 ───────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_scatter_plot(ax,
    x_data=[2, 4, 6], y_data=[3, 3, 5],
    xmin=0, xmax=8, ymin=0, ymax=7,
    xlabel='Input', ylabel='Output', title='Allowed')
save_graph(fig, 'u1_2_mn3_allowed_B.png')
plt.close(fig)

# ── 6: Math Notes 3 — Not Allowed B: input 3 maps to both 2 and 7 ───────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_scatter_plot(ax,
    x_data=[3, 3, 5], y_data=[2, 7, 4],
    xmin=0, xmax=7, ymin=0, ymax=9,
    color='firebrick',
    xlabel='Input', ylabel='Output', title='Not Allowed')
save_graph(fig, 'u1_2_mn3_not_allowed_B.png')
plt.close(fig)

# ── 7: Example 2 — A(b)=2b+7 vs B(b)=5b+1, intersect at b=2 ────────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_context_graph(ax,
    functions=[
        {'expr': lambda b: 2*b + 7,
         'deriv': lambda b: np.ones_like(b) * 2,
         'color': 'steelblue', 'label': r'$A(b)=2b+7$'},
        {'expr': lambda b: 5*b + 1,
         'deriv': lambda b: np.ones_like(b) * 5,
         'color': 'firebrick', 'label': r'$B(b)=5b+1$'},
    ],
    xmin=0, xmax=5, ymin=0, ymax=30,
    xlabel='Books (b)', ylabel='Points')
save_graph(fig, 'u1_2_ex2_clubs_A.png')
plt.close(fig)

# ── 8: YTI 2 — H(x)=4x-2 vs K(x)=x+8, intersect at x=10/3 ─────────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_standard_graph(ax,
    functions=[
        {'expr': lambda x: 4*x - 2,
         'deriv': lambda x: np.ones_like(x) * 4,
         'color': 'steelblue', 'label': r'$H(x)=4x-2$'},
        {'expr': lambda x: x + 8,
         'deriv': lambda x: np.ones_like(x),
         'color': 'firebrick', 'label': r'$K(x)=x+8$'},
    ])
save_graph(fig, 'u1_2_yti2_apps_A.png')
plt.close(fig)

# ── 9: Example 4 — Library P(b)=12+4b ────────────────────────────────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_context_graph(ax,
    functions=[
        {'expr': lambda b: 12 + 4*b,
         'deriv': lambda b: np.ones_like(b) * 4,
         'color': 'steelblue'},
    ],
    xmin=0, xmax=12, ymin=0, ymax=64,
    xlabel='Books (b)', ylabel='Points',
    title=r'$P(b)=12+4b$')
save_graph(fig, 'u1_2_ex4_library_A.png')
plt.close(fig)

# ── 10: Blank standard coordinate plane ──────────────────────────────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_standard_graph(ax, functions=[])
save_graph(fig, 'u1_2_blank_coord_A.png')
plt.close(fig)
...
ax.set_facecolor('white')
make_standard_graph(ax,
    functions=[{'expr': lambda x: 2*x + 1,
                'deriv': lambda x: np.ones_like(x) * 2,
                'color': 'steelblue'}],
    title=r'$f(x)=2x+1$')
save_graph(fig, 'u1_2_func_linear_A.png')
plt.close(fig)
...
ax.set_facecolor('white')
make_standard_graph(ax,
    functions=[{'expr': lambda x: -3*x + 5,
                'deriv': lambda x: np.ones_like(x) * -3,
                'color': 'steelblue'}],
    title=r'$f(x)=-3x+5$')
save_graph(fig, 'u1_2_func_linear_B.png')
plt.close(fig)
...
ax.set_facecolor('white')
make_standard_graph(ax,
    functions=[{'expr': lambda x: 0.5*x - 2,
                'deriv': lambda x: np.ones_like(x) * 0.5,
                'color': 'steelblue'}],
    title=r'$f(x)=\frac{1}{2}x-2$')
save_graph(fig, 'u1_2_func_linear_C.png')
plt.close(fig)
...
ax.set_facecolor('white')
make_standard_graph(ax,
    functions=[{'expr': lambda x: -x + 4,
                'deriv': lambda x: np.ones_like(x) * -1,
                'color': 'steelblue'}],
    title=r'$f(x)=-x+4$')
save_graph(fig, 'u1_2_func_linear_D.png')
plt.close(fig)
...
ax.set_facecolor('white')
make_standard_graph(ax,
    functions=[{'expr': lambda x: 3*x - 2,
                'deriv': lambda x: np.ones_like(x) * 3,
                'color': 'steelblue'}],
    title=r'$f(x)=3x-2$')
save_graph(fig, 'u1_2_func_linear_E.png')
plt.close(fig)
...
ax.set_facecolor('white')
make_standard_graph(ax,
    functions=[{'expr': lambda x: -0.5*x + 3,
                'deriv': lambda x: np.ones_like(x) * -0.5,
                'color': 'steelblue'}],
    title=r'$f(x)=-\frac{1}{2}x+3$')
save_graph(fig, 'u1_2_func_linear_F.png')
plt.close(fig)

# ── 17: Bike rental C(h) = 6 + 4h ───────────────────────────────────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_context_graph(ax,
    functions=[
        {'expr': lambda h: 6 + 4*h,
         'deriv': lambda h: np.ones_like(h) * 4,
         'color': 'steelblue'},
    ],
    xmin=0, xmax=8, ymin=0, ymax=40,
    xlabel='Hours (h)', ylabel='Cost (dollars)',
    title=r'$C(h)=6+4h$')
save_graph(fig, 'u1_2_context_bike_A.png')
plt.close(fig)

# ── 18: YTI 1 mystery function scatter with trend line ───────────────────────
# Points (1,7),(2,9),(4,13) — pattern is output = 2·input + 5
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_scatter_plot(ax,
    x_data=[1, 2, 4], y_data=[7, 9, 13],
    xmin=0, xmax=6, ymin=0, ymax=16,
    line_of_best_fit=True,
    xlabel='Input', ylabel='Output',
    title='Mystery Function')
save_graph(fig, 'u1_2_func_dots_A.png')
plt.close(fig)

# ── 19: Relation C — a function, linear pattern ──────────────────────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_scatter_plot(ax,
    ...
    xlabel='Input', ylabel='Output', title='Relation C')
save_graph(fig, 'u1_2_relation_C_sq.png')
plt.close(fig)

# ── 20: Relation D — NOT a function, input 1 maps to 5 and 2 ────────────────
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
ax.set_facecolor('white')
make_scatter_plot(ax,
    x_data=[0, 1, 1, 2], y_data=[3, 5, 2, 4],
    xmin=0, xmax=4, ymin=0, ymax=7,
    color='firebrick',
    xlabel='Input', ylabel='Output', title='Relation D')
save_graph(fig, 'u1_2_relation_D_sq.png')
plt.close(fig)
