...
# Appended Unit 1 notes graph-generation blocks — Section 1.5
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_context_graph(ax, [
    {'expr': lambda x: 10 + 2*x, 'deriv': lambda x: 0*x + 2, 'color':'steelblue', 'label':'A'},
    {'expr': lambda x: 4*x, 'deriv': lambda x: 0*x + 4, 'color':'firebrick', 'label':'B'},
], xmin=0, xmax=8, ymin=0, ymax=34, xlabel='Input', ylabel='Value', title='')
save_graph(fig, 'u1_notes_s5_balance.png')
plt.close(fig)
