...
# Unit 8 Section 8.3 Notes-specific figure.
# Generated with the authoritative Framework graph tool; no styling overrides.
if __name__ == '__main__':
    f1 = [{'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x < 0, -1.0, 1.0), 'color': 'steelblue'}]
    f2 = [{'expr': lambda x: 2*np.abs(x-1)+1, 'deriv': lambda x: np.where(x < 1, -2.0, 2.0), 'color': 'steelblue'}]
    fig = make_2x1_grid([f1, f2], titles=['',''])
    fig.savefig(os.path.join(OUTPUT_DIR, 'u8_s3_notes_abs_pair.png'), dpi=144, bbox_inches='tight')
    plt.close(fig)
    print('Saved:', os.path.join(OUTPUT_DIR, 'u8_s3_notes_abs_pair.png'))
