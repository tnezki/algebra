...
def _ctx(fn, filename, points=None, xmax=10, ymax=80, xlabel='x', ylabel='y'):
    ...
    save_graph(fig, filename)
    plt.close(fig)
_ctx([{'expr':lambda x: -4*(x-5)**2+60, 'deriv':lambda x: -8*(x-5), 'color':'steelblue', 'label':None}], 'u6_bank_s4_dok4_1_sq_v1.png', xmax=10, ymax=70)

_std([{'expr':lambda x: (x-2)*(x-6), 'deriv':lambda x: 2*x-8, 'color':'steelblue', 'label':None}], 'u6_s4_mn1_method_graph.png', [(2,0),(6,0)])
_std([{'expr':lambda x: (x-4)**2+3, 'deriv':lambda x: 2*(x-4), 'color':'steelblue', 'label':None}], 'u6_s4_mn2_vertex_no_roots.png', [(4,3)])
