...
# Unit 8 notes custom graph-generation blocks

def _notes_standard_mark(filename, functions, points=None, title=''):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_standard_graph(ax, functions, title=title)
    if points:
        for x0, y0 in points:
            ax.plot(x0, y0, 'o', markersize=7, markerfacecolor='white', markeredgecolor='black', markeredgewidth=2, zorder=6)
    save_graph(fig, filename)
    plt.close(fig)

_notes_standard_mark('u8_notes_s1_patterns_sq_v1.png', [
    {'expr': lambda x: 2*x+1, 'deriv': lambda x: 0*x+2, 'color':'steelblue', 'label':'A'},
    {'expr': lambda x: (x-1)**2-4, 'deriv': lambda x: 2*(x-1), 'color':'firebrick', 'label':'B'},
    {'expr': lambda x: 2*(1.35**x)-3, 'deriv': lambda x: np.log(1.35)*2*(1.35**x), 'color':'darkorange', 'label':'C'}
], [(0,1),(1,3),(2,5)], '')
_notes_standard_mark('u8_notes_s1_quad_evidence_sq_v1.png', [
    {'expr': lambda x: x**2 + 2*x + 2, 'deriv': lambda x: 2*x+2, 'color':'firebrick', 'label':'model'}
], [(0,2),(1,5),(2,10),(3,17)], '')
_notes_standard_mark('u8_notes_s2_translate_quad_sq_v1.png', [
    {'expr': lambda x: x**2, 'deriv': lambda x: 2*x, 'color':'steelblue', 'label':'parent'},
    {'expr': lambda x: (x-2)**2-3, 'deriv': lambda x: 2*(x-2), 'color':'firebrick', 'label':'transformed'}
], [(0,0),(2,-3)], '')
_notes_standard_mark('u8_notes_s2_abs_reflect_sq_v1.png', [
    {'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x < 0, -1, 1), 'color':'steelblue', 'label':'parent'},
    {'expr': lambda x: -np.abs(x-1)+4, 'deriv': lambda x: np.where(x < 1, 1, -1), 'color':'firebrick', 'label':'transformed'}
], [(0,0),(1,4)], '')
_notes_standard_mark('u8_notes_s3_abs_vertex_sq_v1.png', [
    {'expr': lambda x: 2*np.abs(x-3)+1, 'deriv': lambda x: np.where(x < 3, -2, 2), 'color':'steelblue', 'label':'distance'}
], [(3,1)], '')
_notes_standard_mark('u8_notes_s3_abs_solve_graph_sq_v1.png', [
    {'expr': lambda x: np.abs(x-4), 'deriv': lambda x: np.where(x < 4, -1, 1), 'color':'steelblue', 'label':'left side'},
    {'expr': lambda x: 0*x+9, 'deriv': lambda x: 0*x, 'color':'firebrick', 'label':'target'}
], [(-5,9),(13,9)], '')
_notes_standard_mark('u8_notes_s3_abs_table_graph_sq_v1.png', [
    {'expr': lambda x: np.abs(x+1)+2, 'deriv': lambda x: np.where(x < -1, -1, 1), 'color':'steelblue', 'label':'rule'}
], [(-1,2),(0,3),(1,4)], '')
_scatter('u8_notes_s4_scatter_positive_sq_v1.png', [1,2,3,4,5,6], [42,48,55,61,70,76], 0, 7, 35, 82, 'Practice hours', 'Score', '', True)
_scatter('u8_notes_s4_model_fit_sq_v1.png', [0,2,4,6,8,10,12], [8,17,27,37,44,47,48], 0, 12, 0, 55, 'Day', 'Height', '', True)
fig = make_2x2_grid([
    [{'expr': lambda x: x+1, 'deriv': lambda x: 0*x+1, 'color':'steelblue', 'label':'A'}],
    [{'expr': lambda x: 0.25*x**2-3, 'deriv': lambda x: 0.5*x, 'color':'firebrick', 'label':'B'}],
    [{'expr': lambda x: 2*(1.25**x)-4, 'deriv': lambda x: np.log(1.25)*2*(1.25**x), 'color':'darkorange', 'label':'C'}],
    [{'expr': lambda x: np.abs(x-2)-3, 'deriv': lambda x: np.where(x < 2, -1, 1), 'color':'steelblue', 'label':'D'}]
], [r'Panel A', r'Panel B', r'Panel C', r'Panel D'])
save_graph(fig, 'u8_notes_s5_family_compare_sq_v1.png')
plt.close(fig)
_notes_standard_mark('u8_notes_s5_intersections_sq_v1.png', [
    {'expr': lambda x: x+2, 'deriv': lambda x: 0*x+1, 'color':'steelblue', 'label':'f'},
    {'expr': lambda x: x**2-4, 'deriv': lambda x: 2*x, 'color':'firebrick', 'label':'g'}
], [(-2,0),(3,5)], '')
