...
# Unit 3 notes figure generation

def _notes_std_point(filename, functions, point=None, label=None):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_standard_graph(ax, functions, title='')
    if point is not None:
        ax.plot([point[0]], [point[1]], marker='o', color='black', markersize=6, zorder=5)
        if label:
            ax.text(point[0] + 0.35, point[1] + 0.35, label, fontsize=11, fontweight='bold', fontfamily='Times New Roman')
    save_graph(fig, filename)
    plt.close(fig)

def _notes_context_point(filename, functions, xmin, xmax, ymin, ymax, xlabel, ylabel, point=None, label=None):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_context_graph(ax, functions, xmin, xmax, ymin, ymax, xlabel=xlabel, ylabel=ylabel, title='')
    if point is not None:
        ax.plot([point[0]], [point[1]], marker='o', color='black', markersize=6, zorder=5)
        if label:
            ax.text(point[0] + (xmax-xmin)*0.03, point[1] + (ymax-ymin)*0.04, label, fontsize=10, fontweight='bold', fontfamily='Times New Roman')
    save_graph(fig, filename)
    plt.close(fig)

def _notes_ineq(filename, inequalities):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_inequality_graph(ax, inequalities, title='')
    save_graph(fig, filename)
...
_notes_std_point('u3_s1_mn1_intersection.png', [
    {'expr': lambda x: x + 2, 'deriv': lambda x: np.full_like(x, 1.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: -1*x + 8, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'firebrick', 'label': 'B'},
], (3, 5), '(3,5)')
_notes_std_point('u3_s1_mn2_graphing_system.png', [
    {'expr': lambda x: 2*x - 2, 'deriv': lambda x: np.full_like(x, 2.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: -1*x + 7, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'firebrick', 'label': 'B'},
], (3, 4), '(3,4)')
_notes_std_point('u3_s2_mn2_substitution_intersection.png', [
    {'expr': lambda x: x + 5, 'deriv': lambda x: np.full_like(x, 1.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: -2*x + 17, 'deriv': lambda x: np.full_like(x, -2.0), 'color': 'firebrick', 'label': 'B'},
], (4, 9), '(4,9)')
_notes_context_point('u3_s3_mn3_break_even.png', [
    {'expr': lambda x: 4 + 2.5*x, 'deriv': lambda x: np.full_like(x, 2.5), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: 16 + 1.5*x, 'deriv': lambda x: np.full_like(x, 1.5), 'color': 'firebrick', 'label': 'B'},
], 0, 20, 0, 60, 'Miles', 'Cost', (12, 34), '(12,34)')
_notes_context_point('u3_s3_mn4_plan_graph.png', [
    {'expr': lambda x: 20 + 12*x, 'deriv': lambda x: np.full_like(x, 12.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: 60 + 4*x, 'deriv': lambda x: np.full_like(x, 4.0), 'color': 'firebrick', 'label': 'B'},
], 0, 10, 0, 150, 'Input', 'Cost', (5, 80), '(5,80)')
_notes_context_point('u3_s3_ex4_plans.png', [
    {'expr': lambda x: 20 + 12*x, 'deriv': lambda x: np.full_like(x, 12.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: 60 + 4*x, 'deriv': lambda x: np.full_like(x, 4.0), 'color': 'firebrick', 'label': 'B'},
], 0, 10, 0, 150, 'Input', 'Cost', (5, 80), '(5,80)')
_notes_context_point('u3_s3_yti4_plans.png', [
    {'expr': lambda x: 15 + 8*x, 'deriv': lambda x: np.full_like(x, 8.0), 'color': 'steelblue', 'label': 'A'},
    {'expr': lambda x: 45 + 3*x, 'deriv': lambda x: np.full_like(x, 3.0), 'color': 'firebrick', 'label': 'B'},
], 0, 10, 0, 120, 'Input', 'Cost', (6, 63), '(6,63)')
_notes_ineq('u3_s4_mn2_ineq.png', [
    {'expr': lambda x: x + 2, 'deriv': lambda x: np.full_like(x, 1.0), 'color': 'steelblue', 'shade': 'below', 'inclusive': True},
])
_notes_ineq('u3_s4_mn4_wrong.png', [
    {'expr': lambda x: -1*x + 4, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'darkorange', 'shade': 'above', 'inclusive': True},
])
_notes_ineq('u3_s4_ex4_wrong.png', [
    {'expr': lambda x: -1*x + 5, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'darkorange', 'shade': 'below', 'inclusive': True},
])
_notes_ineq('u3_s4_yti4_wrong.png', [
    {'expr': lambda x: 2*x - 4, 'deriv': lambda x: np.full_like(x, 2.0), 'color': 'darkorange', 'shade': 'above', 'inclusive': False},
])
_notes_ineq('u3_s5_mn2_system.png', [
    {'expr': lambda x: x + 1, 'deriv': lambda x: np.full_like(x, 1.0), 'color': 'steelblue', 'shade': 'above', 'inclusive': True},
    {'expr': lambda x: -1*x + 7, 'deriv': lambda x: np.full_like(x, -1.0), 'color': 'firebrick', 'shade': 'below', 'inclusive': True},
])
