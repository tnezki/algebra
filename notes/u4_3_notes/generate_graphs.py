...
fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: np.abs(x), 'deriv': lambda x: np.where(x < 0, -1, 1), 'color': 'steelblue', 'label': 'Parent'}, {'expr': lambda x: -2*np.abs(x), 'deriv': lambda x: np.where(x < 0, 2, -2), 'color': 'firebrick', 'label': 'Transformed'}], title='')
for _x, _y in [(0, 0), (1, -2), (-1, -2)]:
    ax.plot(_x, _y, 'o', markersize=7, markerfacecolor='white', markeredgecolor='firebrick', markeredgewidth=2, zorder=6)
save_graph(fig, 'u4_s3_mn_vertical_transform.png')
plt.close(fig)

_std_fig('u4_s3_blank_notes.png')


fig, ax = plt.subplots(figsize=(3.5, 3.5))
fig.patch.set_facecolor('white')
make_standard_graph(ax, [{'expr': lambda x: -2*np.abs(x-1)+5, 'deriv': lambda x: np.where(x < 1, 2, -2), 'color': 'steelblue'}], title='')
for _x, _y in [(1, 5), (2, 3), (3, 1), (0, 3)]:
    ax.plot(_x, _y, 'o', markersize=7, markerfacecolor='white', markeredgecolor='firebrick', markeredgewidth=2, zorder=6)
save_graph(fig, 'u4_s3_mn_mismatch_check.png')
plt.close(fig)
