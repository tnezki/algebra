...
# Unit 4 notes-specific graph-generation blocks appended below the bank graph section.

def _std_fig_marked(filename, functions=None, title='', points=None, labels=None):
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    fig.patch.set_facecolor('white')
    make_standard_graph(ax, functions or [], title=title)
    if points:
        for idx, (px, py) in enumerate(points):
            ax.plot(px, py, 'o', markersize=6, markerfacecolor='black', markeredgecolor='black', zorder=7, clip_on=False)
            if labels and idx < len(labels) and labels[idx]:
                ax.text(px + 0.35, py + 0.35, labels[idx], fontsize=10, fontfamily='Times New Roman', fontweight='bold')
    save_graph(fig, filename)
    plt.close(fig)

_std_fig_marked('u4_notes_s2_abs_translation_marked.png', [
    {'expr': lambda x: np.abs(x-3) + 2, 'deriv': lambda x: np.where(x < 3, -1, 1), 'color': 'steelblue'}
], points=[(3,2),(2,3),(4,3)], labels=[r'$(3,2)$', r'$(2,3)$', r'$(4,3)$'])

_std_fig_marked('u4_notes_s2_quad_translation_marked.png', [
    {'expr': lambda x: (x+2)**2 - 3, 'deriv': lambda x: 2*(x+2), 'color': 'steelblue'}
], points=[(-2,-3),(-1,-2),(-3,-2)], labels=[r'$(-2,-3)$', r'$(-1,-2)$', r'$(-3,-2)$'])

_std_fig_marked('u4_notes_s3_abs_transform_marked.png', [
    {'expr': lambda x: -2*np.abs(x+1) + 3, 'deriv': lambda x: np.where(x < -1, 2, -2), 'color': 'steelblue'}
], points=[(-1,3),(-2,1),(0,1)], labels=[r'$(-1,3)$', r'$(-2,1)$', r'$(0,1)$'])

_std_fig_marked('u4_notes_s3_quad_transform_marked.png', [
    {'expr': lambda x: -0.5*(x-2)**2 + 4, 'deriv': lambda x: -(x-2), 'color': 'steelblue'}
], points=[(2,4),(0,2),(4,2)], labels=[r'$(2,4)$', r'$(0,2)$', r'$(4,2)$'])

_std_fig_marked('u4_notes_s5_vertex_features_marked.png', [
    {'expr': lambda x: (x-2)**2 - 4, 'deriv': lambda x: 2*(x-2), 'color': 'steelblue'}
], points=[(2,-4),(0,0),(4,0)], labels=[r'$(2,-4)$', r'$(0,0)$', r'$(4,0)$'])

_std_fig_marked('u4_notes_s5_feature_example.png', [
    {'expr': lambda x: (x+1)*(x-5), 'deriv': lambda x: 2*x-4, 'color': 'steelblue'}
], points=[(-1,0),(5,0),(2,-9)], labels=[r'$(-1,0)$', r'$(5,0)$', r'$(2,-9)$'])

_std_fig_marked('u4_notes_s5_feature_yti.png', [
    {'expr': lambda x: -(x-1)*(x-5), 'deriv': lambda x: -2*x+6, 'color': 'steelblue'}
], points=[(1,0),(5,0),(3,4)], labels=[r'$(1,0)$', r'$(5,0)$', r'$(3,4)$'])
